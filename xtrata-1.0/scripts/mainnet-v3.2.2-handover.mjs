...
const CORE_NAME = process.env.XTRATA_CORE_NAME ?? 'xtrata-v3-2-2';
const REQUIRED_CLARITY_VERSION = 3;
const CORE_SOURCE_PATH = 'contracts/live/xtrata-v3.2.2.clar';
const ANNOUNCEMENT_PATH = 'docs/mainnet-v3.2.2-announcement-inscription.md';
const REPORT_JSON = path('reports/mainnet-v3.2.2-handover.json');
const REPORT_MD = path('reports/mainnet-v3.2.2-handover.md');
...
const inspectSource = (source) => {
  ...
  const asContractInFunctionBody = source.split('\n').some((line) => {
  ...
  if (asContractInFunctionBody) {
    failures.push('Core source contains (as-contract tx-sender) outside a constant definition.');
  }
  if (!source.includes('"xtrata-v3.2.2"')) {
    failures.push('Core source version string does not match xtrata-v3.2.2.');
  }
  ...
  if (source.includes('(define-public (migrate-from-v2-1-1')) {
    failures.push('Core source still contains migrate-from-v2-1-1, which must be removed in v3.2.2.');
  }
...
const computeAnnouncement = (sourceText) => {
  ...
  return {
    ...
    mime: 'text/markdown',
    tokenUri: `${DEPLOYER}.xtrata-v3-2-2`,
...
async function verify(txid) {
  if (!txid || !/^0x[0-9a-f]{64}$/i.test(txid)) {
    throw new Error('Usage: npm run mainnet:v3.2.2:verify -- 0x<64-character-txid>');
  }
  const { response, text } = await fetchWithRetry(
...
function printReport() {
  if (!existsSync(REPORT_JSON)) {
    throw new Error('No v3.2.2 handover report exists. Run the preflight first.');
  }
