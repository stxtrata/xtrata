...
const logsKey = 'xtrata-mainnet-v3.2.2-handover-txs';
...
const setNextId = () => {
  ...
  if (!report.setNextIdReady) {
    throw new Error(
      'set-next-id is blocked by this report. Pause the current live core, wait for confirmation, run npm run mainnet:v3.2.2:preflight, then refresh this page.'
    );
  }
  return callWallet(`set v3.2.2 next-id ${report.recommendedNextId}`, {
    contractAddress: report.core.address,
    contractName: report.core.name,
...
const unpauseCore = () => {
  if (!report) return Promise.resolve();
  return callWallet('unpause xtrata-v3-2-2', {
    contractAddress: report.core.address,
    contractName: report.core.name,
    functionName: 'set-paused',
...
const mintAnnouncement = () => {
  if (!report) return Promise.resolve();
  return callWallet('mint v3.2.2 announcement inscription', {
    contractAddress: report.core.address,
    contractName: report.core.name,
...
const render = () => {
  ...
  app.innerHTML = `
    ...
    <section class="panel">
      ...
      ${[
        ['deploy', 'Deploy xtrata-v3-2-2', 'Deploys as Clarity 3. Skip only if already deployed and source-hash verified.'],
        ['pause', 'Pause current live v2.1.0 core', 'Freezes new v2 native mints while preserving transfer and migration safety.'],
        ['next', `Set v3.2.2 next-id to ${report.recommendedNextId ?? '?'}`, 'One-shot continuity step. Requires a fresh report showing the current live core is paused.'],
        ['royalty', `Set royalty recipient to ${report.royaltyRecipient}`, 'Confirms protocol fee recipient before public use.'],
        ['unpause', 'Unpause v3.2.2 core', 'Enables the new core for minting.'],
...
const load = async () => {
  report = await fetch('/reports/mainnet-v3.2.2-handover.json').then((entry) => {
    if (!entry.ok) {
      throw new Error('Run npm run mainnet:v3.2.2:preflight before opening the handover UI.');
    }
