...
const badThumbnailKeys = new Set<string>();
...
export default function TokenCardMedia(props: TokenCardMediaProps) {
  ...
  const [forceTokenUriFallback, setForceTokenUriFallback] = useState(false);
  ...
  const thumbnailKey = `${props.contractId}:${props.token.id.toString()}`;
  const thumbnailQuery = useQuery({
    queryKey: getTokenThumbnailKey(props.contractId, props.token.id),
    queryFn: () =>
      badThumbnailKeys.has(thumbnailKey)
  ...
  useEffect(() => {
    ...
    setForceTokenUriFallback(false);
  ...
  const handleImageError = useCallback(
    (event: SyntheticEvent<HTMLImageElement>) => {
      ...
      if (imagePreviewOrigin === 'thumbnail-cache') {
        ...
        badThumbnailKeys.add(thumbnailKey);
        queryClient.setQueryData(
          getTokenThumbnailKey(props.contractId, props.token.id),
          null
        );
      ...
      const shouldForceFallback =
        !!props.token.tokenUri &&
        !forceTokenUriFallback &&
        !imagePreviewOrigin.startsWith('token-uri');
      if (shouldForceFallback) {
        setForceTokenUriFallback(true);
      }
      ...
      if (shouldForceFallback) {
        diagnosticHints.push('token-uri-fallback-triggered');
      }
      if (
        imagePreviewSource.startsWith('blob:') &&
