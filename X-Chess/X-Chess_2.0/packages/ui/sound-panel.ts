// The controls, generated from the library.
//
// One row per entry in VOICES, built here rather than written into the shell.
// Adding a sound to the table adds its row, its switch, its slider and its
// preview button, and there is no second list to forget to update. The same
// argument as the piece sizing CSS: a table and the thing it drives cannot
// disagree if only one of them is written by hand.
//
// Built with document methods and never with innerHTML. Every label in here
// comes from the table, and the day somebody puts an apostrophe or a bracket in
// one of those strings is not the day the sound panel should break.

import { SOUND_EVENTS, VOICES } from './sounds.js';
...
export function renderSoundPanel(root: HTMLElement, sound: Sound): SoundPanel {
  ...
  const switches: { event: SoundEvent; on: HTMLInputElement; gain: HTMLInputElement }[] = [];
  ...
  for (const event of SOUND_EVENTS) {
    ...
    name.appendChild(on);
    name.appendChild(document.createTextNode(` ${voice.label}`));
    ...
    gain.setAttribute('aria-label', `${voice.label} volume`);
    ...
    // Previewing plays the sound even when its switch is off, so somebody can
    // hear what they are turning on before they turn it on. It doubles as the
    // gesture that unlocks the audio context.
    test.setAttribute('aria-label', `Play the ${voice.label} sound`);
    test.addEventListener('click', () => sound.preview(event));

    const say = document.createElement('div');
    say.className = 'snd-say small muted';
    say.textContent = voice.say;

    row.append(name, gain, test, say);
    root.appendChild(row);
    switches.push({ event, on, gain });
  }
 ...
 * browser that has not been touched yet will not make a sound, and a person
 * looking at a panel that says everything is switched on has no way to know
 * that. Silence with no explanation reads as a broken feature.
 */
export function soundNote(sound: Sound): string {
  ...
  if (sound.waitingForATap) {
    return 'Your browser will not play a sound until you have touched the page. ' +
      'Press Play on any row above, or make a move, and it will start working.';
  }
  if (sound.background) {
    return 'This board keeps reading the chain while the tab is in the background, so you will ' +
      'hear your turn while you are doing something else. Browsers slow background tabs down, ' +
      'so it can be a little late; it will not be missed.';
  }
  return 'Sounds play while this tab is in front. Turn on background listening to hear your ' +
    'turn while you are looking at something else.';
}
