...
export interface Store {
  ...
  setMany?(rows: Array<[string, string]>): Promise<void>;
}
...
export class IndexedDbStore implements Store {
  ...
  constructor(private readonly name = 'xchess') {}
  private open(): Promise<IDBDatabase | null> {
    if (this.db) return Promise.resolve(this.db);
    if (this.opening) return this.opening;
    this.opening = new Promise<IDBDatabase | null>((done) => {
      let settled = false;
      const finish = (db: IDBDatabase | null): void => { settled = true; clearTimeout(timer); done(db); };
  ...
  private async batch(mode: IDBTransactionMode, run: (store: IDBObjectStore) => IDBRequest[]): Promise<unknown[] | null> {
    const db = await this.open();
    if (!db) return null;
    return new Promise((done) => {
      try {
        const tx = db.transaction('kv', mode);
        const requests = run(tx.objectStore('kv'));
        // A request succeeding is not a committed write. Wait for the transaction.
        tx.oncomplete = () => done(requests.map(r => r.result));
        tx.onabort = tx.onerror = () => done(null);
      } catch { done(null); }
    });
  }
  async getMany(keys: string[]): Promise<Array<string | null>> {
    const rows = await this.batch('readonly', store => keys.map(key => store.get(key)));
    return keys.map((_, i) => typeof rows?.[i] === 'string' ? rows[i] as string : null);
  }
  async setMany(rows: Array<[string, string]>): Promise<void> {
    if (rows.length) await this.batch('readwrite', store => rows.map(([key, value]) => store.put(value, key)));
  }
  async get(key: string): Promise<string | null> { return (await this.getMany([key]))[0]; }
  async set(key: string, value: string): Promise<void> { await this.setMany([[key, value]]); }
  async clear(): Promise<void> { await this.batch('readwrite', store => [store.clear()]); }
}

/** Shape checks only: entries may contain invalid chess, which replay must still see. */
function cachedEntry(raw: string | null, seq: number): EntryRow | null {
  try {
    const row = raw === null ? null : JSON.parse(raw);
    return row && row.seq === seq && typeof row.value === 'string' && typeof row.sender === 'string' &&
      Number.isSafeInteger(row.height) && row.height >= 0 ? row as EntryRow : null;
  } catch { return null; }
}
...
export class CachingReader implements ChainReader {
  ...
  private async readMany(keys: string[]): Promise<Array<string | null>> {
    try { return this.store.getMany ? await this.store.getMany(keys) : await Promise.all(keys.map(k => this.store.get(k))); }
    catch { return keys.map(() => null); }
  }
  private async writeMany(rows: Array<[string, string]>): Promise<void> {
    try {
      if (this.store.setMany) await this.store.setMany(rows);
      else await Promise.all(rows.map(([k, v]) => this.store.set(k, v)));
    } catch { /* Quota and restricted storage never prevent a chain read. */ }
  }

  prioritizeGame(game: number): void { this.inner.prioritizeGame?.(game); }

  get network() { return this.inner.network; }
  ...
  private key(kind: string, ...parts: (string | number)[]): string {
    ...
    return `${this.network && this.network !== 'mainnet' ? this.network + '|' : ''}${this.contractId}|${kind}|${parts.join('|')}`;
  }
  ...
  async getEntry(game: number, seq: number): Promise<EntryRow | null> {
    ...
    const cached = cachedEntry((await this.readMany([key]))[0], seq);
    if (cached) {
      this.hits++;
      return cached;
    }
    ...
    if (row) await this.writeMany([[key, JSON.stringify(row)]]);
    return row;
  }
  ...
  async getAllEntries(game: number, knownNextSeq?: number): Promise<EntryRow[]> {
    const out: EntryRow[] = [];
    prefix: for (let start = 0; knownNextSeq === undefined || start < knownNextSeq; start += PAGE_SIZE) {
      const length = Math.min(PAGE_SIZE, knownNextSeq === undefined ? PAGE_SIZE : knownNextSeq - start);
      const cached = await this.readMany(Array.from({length}, (_, i) => this.key('entry', game, start + i)));
      for (let i = 0; i < length; i++) {
        const row = cachedEntry(cached[i], start + i);
        if (!row) break prefix;
        this.hits++;
        out.push(row);
      }
    ...
    for (;;) {
      ...
      for (const row of rows) {
        ...
        out.push(row);
        this.misses++;
        writes.push([this.key('entry', game, row.seq), JSON.stringify(row)]);
      }
      await this.writeMany(writes);
  ...
  async getRankedGame(index: number): Promise<number | null> {
    ...
    const raw = (await this.readMany([key]))[0];
    try {
      const cached = raw === null ? null : JSON.parse(raw);
      if (Number.isSafeInteger(cached) && cached >= 1) {
        this.hits++;
        return cached as number;
      }
    } catch { /* Re-fetch and replace corrupt records. */ }
    this.misses++;
    ...
    if (id !== null) await this.writeMany([[key, JSON.stringify(id)]]);
    return id;
  }
  ...
  async clear(): Promise<void> {
    try { await this.store.clear(); } catch { /* Storage is optional. */ }
    this.hits = 0;
    this.misses = 0;
  }
