...
async function launch(side){const profile=await mkdtemp(join(tmpdir(),'xchess-peer-'+side));profiles.push(profile);const b=spawn(process.env.CHROME_BIN||'google-chrome',['--headless=new','--disable-gpu','--no-first-run','--window-size=390,844',`--user-data-dir=${profile}`,url+'/mobile?side='+side],{stdio:'ignore'});browsers.push(b);await cmd(side,'open');return b;}
...
try{
  ...
  for(const side of ['white','black']){const s=await state(side);assert.equal(s.playableSquares,0);assert.match(s.readiness,/Registration alone does not send moves/);assert.match(s.clocks,/Paused/);assert.match(s.clocks,/White 3:00/);}
  pass('Joined game has an explicit transport choice and no running clock before connection');
  await cmd('white','import',{text:''});assert.match((await state('white')).status,/Paste the complete/);
  await cmd('white','import',{text:'{'});assert.match((await state('white')).status,/incomplete or invalid JSON/);pass('Empty and truncated handoff inputs give actionable messages');
  ...
  await cmd('black','click',{name:'manual'});
  ...
  await cmd('white','click',{name:'manual'});await cmd('black','click',{name:'manual'});
  ...
  for(const side of ['white','black','reader']){const s=await state(side);assert.equal(s.viewportWidth,390);assert.equal(s.boardSquares,64);assert.ok(s.squareHeight*8<=s.boardHeight+2);assert.equal(s.overflow,false);assert.deepEqual(s.errors,[]);assert.deepEqual(s.external,[]);assert.ok(s.pawns.length>0);for(const pawn of s.pawns){assert.equal(pawn.svg,true);assert.equal(pawn.fill,pawn.color==='white'?'rgb(255, 255, 255)':'rgb(23, 19, 15)');assert.ok(pawn.height>0);assert.ok(pawn.height<s.squareHeight);}const k=await cmd(side,'keyboard');assert.ok(k.active?.startsWith('peer-board-square-'));assert.equal(k.focus,true);}
  pass('Embedded SVG pawns have the correct white/black fills at mobile board size');
