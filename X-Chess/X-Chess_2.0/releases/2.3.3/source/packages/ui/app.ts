import { renderFeeAdvice, MOVE_FEE_STX } from './fee-advice.js';
import { applyGamePreset } from './game-presets.js';
import { factsScope } from '../chain/game-facts.js';
import { readRatingCache, writeRatingCache, ratingGames } from './rating-state.js';
// The application.
//
...
const IDS = [
  ...
  'game-preset', 'game-kind', 'rules-white', 'rules-black', 'rules-ranked',
...
export class ChessApp {
  ...
  private exploreLoading = false;
  private leaderboardLoading = false;
  // In-memory completed replays only; each reuse checks a fresh chain row.
  private rankedReplays = new Map<number, { stamp: string; state: ReplayState }>();
  ...
  private wire(): void {
    ...
    on('leaderboardVerify', () => {
      ...
      if (!this.verifyEverything) this.rankedReplays.clear();
    ...
    this.doc.addEventListener('keydown', (event) => {
      ...
      if (event.defaultPrevented || typing || target?.closest?.('button, dialog, [role=dialog], [role=grid]') || this.tab !== 'game' || !this.canReplay()) return;
    ...
    this.el.gamePreset.addEventListener('change', () => {
      applyGamePreset(this.doc, (this.el.gamePreset as HTMLSelectElement).value, this.address);
      this.drawSeatFields();
      this.drawDraft();
    });
    for (const key of [
      ...
      const redraw = (): void => {
        (this.el.gamePreset as HTMLSelectElement).value = 'custom';
        this.drawSeatFields();
        this.drawDraft();
      };
  ...
  private async checkContract(): Promise<void> {
    ...
    try {
      ...
      this.notice('chainNotice', 'info', `Reading ${this.chain.contractId} (format ${version}). Open a game number or choose Start a game.`);
      ...
      if (revision === this.noticeRevision) this.reportFailure('reading the contract', error);
    }
  }
  ...
  private guardedReads = 0;
  private chainReadFailed = false;

  private notice(key: string, kind: 'info' | 'warn' | 'good' | 'loud', message: string): void {
    if (key === 'chainNotice') this.noticeRevision++;
  ...
  private async guard<T>(what: string, run: () => Promise<T>): Promise<T | null> {
    ...
    try {
      ...
      if (result !== null && result !== false && this.chainReadFailed && revision === this.noticeRevision) {
        this.chainReadFailed = false;
        this.notice('chainNotice', 'good', `Chain reads recovered. Finished ${what}.`);
      }
      return result;
    } catch (error) {
      if (revision === this.noticeRevision) this.reportFailure(what, error);
      if (what === 'computing ratings') {
        this.el.leaderboardVerify.classList.remove('hide');
        this.el.leaderboardVerify.textContent = 'Retry verification';
        if (this.progress?.what === 'ranked games replayed') this.progress = null;
        this.notice('leaderboardNote', 'warn', 'Verification stopped before every game could be read. The previous table has not been replaced. Use Retry verification when the endpoint is available. Completed verified replays are retained and checked against fresh game rows.');
      }
      return null;
    } finally {
      this.busy = --this.guardedReads > 0;
    }
  }
  ...
  private reportFailure(what: string, error: unknown): void {
    ...
    const code = err?.code;
    if (code === 'RATE_LIMITED' || code === 'CHAIN_UNAVAILABLE' || code === 'NO_ENDPOINT') this.chainReadFailed = true;
  ...
  async load(game: number): Promise<void> {
    this.chain.prioritizeGame?.(game);
    const revision = ++this.loadRevision;
    this.notice('chainNotice', 'info', `Looking for game ${game} on ${this.chain.contractId}.`);
    ...
    const loaded = await this.guard(`loading game ${game}`, async () => {
      ...
      if (!row) {
        this.notice('chainNotice', 'warn', `There is no game ${game}. Open another game number or start a game.`);
        return false;
      }
      const [entries, read] = await Promise.all([
        this.chain.getAllEntries(game, row.nextSeq),
    ...
    if (!loaded || revision !== this.loadRevision) return;
    if (noticeRevision === this.noticeRevision) this.notice('chainNotice', 'good', `Game ${game} loaded from the chain.`);
  ...
  private async adoptWithManifests(): Promise<void> {
    ...
    if (this.rulesConfirmed || this.gameId !== game || this.loadRevision !== revision) return;
    this.adoptRules();
  ...
  private drawFeeAdvice(): void {
    ...
    const key = stuck ? `${stuck.txid}:${stuck.nonce}:${stuck.fee}:${Math.floor(this.now() / 60_000)}` : 'none';
    if (key === this.feeAdviceKey) return;
    this.feeAdviceKey = key;

    renderFeeAdvice(this.el.feeAdvice, this.doc, stuck, this.now());
  }
      ...
      say(
        this.intent.state === 'signing'
          ? `${this.intent.from} to ${this.intent.to} is waiting for your wallet. Nothing has been sent yet. ` +
            ...
            `Your wallet estimates the current network fee. Historical moves paid ` +
            `${MOVE_FEE_STX} STX; that is not a current estimate or a confirmation guarantee.`
...
// ------------------------------------------------------------------
  ...
  private drawTournamentFresh(): void {
    ...
    if (!view?.ok || !view.scored || !this.tournamentReadAt) {
  ...
  private tournamentState(id: number): TournamentState {
    try {
      const raw = (globalThis as { localStorage?: Storage }).localStorage?.getItem(STATE_KEY + 'v2:' + factsScope(this.chain) + ':' + id);
  ...
  private rememberTournamentState(view: TournamentView): void {
    ...
    const state: TournamentState = games.every((g) => g.result !== null)
      ...
      : games.some((g) => (g.moves ?? 0) > 0)
        ...
        : 'planned';
    try {
      (globalThis as { localStorage?: Storage }).localStorage?.setItem(STATE_KEY + 'v2:' + factsScope(this.chain) + ':' + id, state);
    } catch {
...
/**
  ...
  private loadRatingCache(): void {
    const cache = readRatingCache(this.chain.contractId);
    if (!cache) return;
  ...
  private saveRatingCache(rows: LeaderboardRow[], atCount: number, open: Array<[number, number]>): void {
    writeRatingCache(this.chain.contractId, rows, atCount, open);
  }
  ...
  private drawTournament(): void {
    ...
    this.drawTournamentFresh();
    if (view?.ok && view.scored) {
      this.rememberTournamentState(view);
      this.drawPickerFilters();
      this.drawTournamentList();
    }
   ...
   */
  private promotionFocus: string | null = null;
  private hidePromotion(): void {
    if (this.el.promotion.contains(this.doc.activeElement)) {
      if (this.promotionFocus) this.el.board.dataset.focusSquare = this.promotionFocus;
  ...
  private askPromotion(choices: string[]): void {
    ...
    this.promotionFocus = pending?.from ?? this.el.board.dataset.focusSquare ?? null;
    node.setAttribute('role', 'dialog');
    node.setAttribute('aria-label', 'Choose promotion piece');
    node.classList.remove('hide');
    node.replaceChildren();
    ...
    node.onkeydown = event => {
      ...
      const at = buttons.indexOf(this.doc.activeElement as HTMLButtonElement);
   ...
   */
  private async loadExplore(requestedTop: number | null = this.exploreTop): Promise<void> {
    if (this.exploreLoading) return;
    this.exploreLoading = true;
    this.text('exploreFound', 'Loading games… The previous page remains visible until this page is ready.');
    this.drawExplore();
    let committed = false;
    try {
    ...
    await this.guard('reading the game list', async () => {
      ...
      const top = Math.min(requestedTop ?? count, count);
      const first = Math.max(1, top - (EXPLORE_WINDOW - 1));
      const ids: number[] = [];
      for (let id = top; id >= first; id--) ids.push(id);
      const onNewest = requestedTop === null || top >= count;
      ...
      if (!committed) this.text('exploreFound', 'Could not load that page. The previous page is unchanged. Retry Older or Newer, or Refresh for the newest games.');
      this.drawExplore();
    }
  }
  ...
  private async pageExplore(direction: -1 | 1): Promise<void> {
    ...
    if (this.exploreLoading || count < 1) return;
    ...
    const requestedTop = next >= count ? null : Math.max(EXPLORE_WINDOW, next);

    this.exploreFound = null;
    this.text('exploreFound', '');
    await this.loadExplore(requestedTop);
  }
  ...
  private drawExplore(): void {
    ...
    const atOldest = Math.max(1, Math.min(this.exploreTop ?? this.exploreTotal, this.exploreTotal)
      - (EXPLORE_WINDOW - 1)) <= 1;
    (this.el.exploreNewer as HTMLButtonElement).disabled = this.exploreLoading || atNewest;
    (this.el.exploreOlder as HTMLButtonElement).disabled = this.exploreLoading || atOldest;
    (this.el.exploreRefresh as HTMLButtonElement).disabled = this.exploreLoading;
  ...
  async loadLeaderboard(): Promise<void> {
    ...
    (this.el.leaderboardVerify as HTMLButtonElement).disabled = true;
    this.notice('leaderboardNote', 'info', 'Preparing verification: reading tournament settings and ranked games.');
    try {
    await this.guard('computing ratings', async () => {
      ...
      const judged = await pool(
        ...
        Array.from({ length: count - from }, (_, at) => async (): Promise<RatedGame | null> => {
          ...
          if (id === null) throw new Error(`Ranked index ${index} is unavailable`);
          const row = await this.chain.getGame(id);
          if (!row) throw new Error(`Ranked game ${id} is unavailable`);

          const stamp = JSON.stringify(row);
          const held = this.rankedReplays.get(id);
          let state: ReplayState;
          if (held?.stamp === stamp) state = held.state;
          else {
            const entries = await this.chain.getAllEntries(id, row.nextSeq);
            if (entries.length !== row.nextSeq) throw new Error(`Incomplete history for ranked game ${id}`);
          ...
          this.rankedReplays.set(id, { stamp, state });
      ...
      const rated = ratingGames(this.checkpointSeed(), walked, from === 0);
      ...
      this.el.leaderboardVerify.textContent = 'Verify from scratch';
      ...
      (this.el.leaderboardVerify as HTMLButtonElement).disabled = false;
    }
  }
  ...
  private manifestPairingsPending: Promise<void> | null = null;
  private async ensureManifestPairings(): Promise<void> {
    if (this.manifestPairingsPending) return this.manifestPairingsPending;
    const pending = this.readManifestPairings();
    this.manifestPairingsPending = pending;
    try { await pending; } finally { this.manifestPairingsPending = null; }
  }
  private async readManifestPairings(): Promise<void> {
  ...
  private drawHelp(): void {
    ...
    body.replaceChildren();
    const peerHelp = this.doc.createElement('p');
    peerHelp.textContent = 'Quick Play uses signed off-chain moves: both named players register once, then connect directly or exchange signed messages. Save an encrypted recovery backup. At checkmate, an automatic draw or signed resignation, either player can download and inscribe the public archive without another signature from the opponent. Clocks are advisory; disconnection or expiry does not prove a win. The manual below describes legacy play, where every move is a transaction.';
    body.appendChild(peerHelp);
