...
/* TWO-TONE, and that is the whole point. --gold #d8a24a against the light square
   ...
   in greyscale. */
.board:focus .sq--focus { outline: 3px solid #ffcf70; outline-offset: -4px; }
.sq--selected {
...
/* SVG pawns retain solid side colours and hollow pending-move outlines on iOS. */
.pc-pawn { width: .7em; height: .84em; display: block; fill: currentColor; stroke: #241c12; stroke-width: 1.3; stroke-linejoin: round; }
.pc--black .pc-pawn { stroke: #a39989; }
.pc--ghost .pc-pawn { fill: none; }
.pc--signing .pc-pawn { stroke: var(--gold); }
.pc--sent .pc-pawn { stroke: var(--good); }
...
<main>
  ...
  <section id="view-play" class="panel">
    <h2>Start a game</h2>
    <div class="field">
      <label for="game-preset">Choose a starting setup</label>
      <select id="game-preset">
        <option value="community">Community game</option>
        <option value="friend">Challenge a friend</option>
        <option value="first-two">First two players</option>
        <option value="custom">Custom setup</option>
      </select>
      <span class="how">Review the seats and rules below before opening your wallet. Existing games cannot change.</span>
    </div>
    <div class="field">
