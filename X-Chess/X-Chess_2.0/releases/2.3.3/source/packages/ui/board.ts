import { wireBoardKeyboard } from './board-keyboard.js';
...
//
// Embedded pawn SVG and text glyphs for the other pieces. An inscription cannot fetch anything, and a sprite
...
export function pieceNode(type: PieceType, color: Color, extra = ''): HTMLElement | null {
  ...
  if (type === PAWN) {
    // A font-independent silhouette: U+265F can become a black emoji on iOS.
    const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    svg.setAttribute('viewBox', '0 0 40 48');
    svg.setAttribute('class', 'pc-pawn');
    svg.setAttribute('focusable', 'false');
    const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    path.setAttribute('d', 'M15 18a8 8 0 1 1 10 0l2 4H13z M15 24h10c0 7 2 10 6 13H9c4-3 6-6 6-13z M8 39h24l2 6H6z');
    svg.append(path);
    node.append(svg);
  } else node.textContent = GLYPH[key];
...
export function renderBoard(root: HTMLElement, view: BoardView, handlers: BoardHandlers): void {
  ...
  const hadFocus = active === root || !!active && root.contains(active);
  const restore = active?.dataset.square ?? root.dataset.focusSquare ?? null;
  ...
  wireBoardKeyboard(root, restore, hadFocus);
}
