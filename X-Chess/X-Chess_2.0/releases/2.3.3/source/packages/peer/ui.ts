...
export class PeerApp {
  ...
  private manual=false;private peerSeen:string|null=null;
  ...
  constructor(doc:Document=document) {
    ...
    this.el('network').addEventListener('change',()=>{const input=this.el<HTMLInputElement>('registry');if(this.value('network')!=='mainnet'&&input.value==='SPARQA0T0GWJZADHRMGNVTJ51D014V8P7XPDSTNH.xchess-peer-v1')input.value='';else if(this.value('network')==='mainnet'&&!input.value)input.value='SPARQA0T0GWJZADHRMGNVTJ51D014V8P7XPDSTNH.xchess-peer-v1';});
    this.on('connect-guide',()=>{this.el('connect-panel').scrollIntoView({block:'start'});this.el('offer').focus();});
    this.on('exchange-guide',()=>{this.el('exchange-panel').scrollIntoView({block:'start'});this.el('input').focus();});
    this.on('manual',()=>{this.transport?.close();this.transport=null;this.peerSeen=null;this.manual=true;this.el('connection').textContent='Manual exchange selected. Send each signed move yourself.';this.notice('Manual exchange enabled. After your move, use Send / receive message to copy and send the saved history.');});
    ...
    this.on('copy',async()=>{const out=this.el<HTMLTextAreaElement>('output');try{await navigator.clipboard.writeText(out.value);this.notice('Message copied. Send it to the other player; copying alone does not deliver it.');}catch{out.focus();out.select();this.notice('Copy the selected text with your browser’s Copy command.');}});
    ...
    this.on('offer',async()=>{const t=await this.connection();this.peerSeen=null;this.output(canonical(await t.offer(this.networkOptions())),'Connection offer — copy and send to the other player; never inscribe');this.notice('Send the offer below. Paste the answer they return, then choose Accept pasted answer.');});
    this.on('answer',async()=>{const input=this.messageInput(this.value('input'),'connection offer');const t=await this.connection();this.peerSeen=null;this.output(canonical(await t.answer(input,this.networkOptions())),'Connection answer — copy and return to the first player; never inscribe');this.notice('Send the answer below back to the player who created the offer.');});
    this.on('accept-answer',async()=>{const input=this.messageInput(this.value('input'),'connection answer');check(this.transport,'Create an offer first');await this.transport.acceptAnswer(input);});
    this.on('disconnect',()=>{this.transport?.close();this.transport=null;this.peerSeen=null;this.el('connection').textContent='Disconnected. The game is saved; manual exchange still works.';});
  ...
  open():void{this.dialog.showModal();if(this.clock)this.clock.seen=performance.now();this.render();}
  ...
  private messageInput(text:string,kind='signed message'):string{
    check(text.trim(),`Paste the complete ${kind} into the input box first.`);
    try{parseBounded(text,MAX_BYTES);}catch(e){if(e instanceof SyntaxError)throw Error(`This ${kind} is incomplete or invalid JSON. Copy the entire message and paste it again. Your saved game is unchanged.`);throw e;}return text;
  }
  async importMessage(text:string):Promise<void>{
    const data=parseBounded(this.messageInput(text),MAX_BYTES) as Archive|Signed<DemoInvite>|Signed<DemoJoin>;
    ...
    if('payload' in data&&data.payload?.format==='xchess-peer-demo-join-v1'){
      ...
      await this.journal.acceptDemo(opening.descriptor.nonce,hash(opening));const game=await this.journal.start(opening);await this.journal.setMeta('actor:'+hash(opening),'white');await this.useGame(game,false,false);this.notice('Demo joined as White. Choose Connect devices or Use manual exchange before moving.');return;
    }
    ...
    if(!this.transport?.connected)this.manual=true;
  ...
  private async useGame(game:Game,verified:boolean,readOnly:boolean,keep=false):Promise<void>{
    ...
    if(changed||!keep){this.generation++;this.transport?.close();this.transport=null;this.manual=false;this.peerSeen=null;this.el('connection').textContent='Devices are not connected. Exchange an offer and answer, or choose manual exchange.';}
    ...
    if(changed||!keep||!this.clock)this.clock={white:game.opening.descriptor.clock.baseMs,black:game.opening.descriptor.clock.baseMs,turn:actorTurn(r.position),seen:performance.now(),running:false};
    else {
    ...
    if(changed||!keep)this.flipped=this.actor==='black';this.render();
  }
  async refresh():Promise<void>{check(this.game,'No active game');const game=await this.journal.get(hash(this.game.opening));check(game,'No saved copy on this device');await this.useGame(game,this.chainVerified,this.readOnly,true);this.notice('This device’s saved history reloaded. To receive the other player’s moves, connect devices or import their signed message.');}
  private canSign():boolean{return !!this.game&&!!this.actor&&!this.readOnly&&(this.game.opening.kind==='demo'||this.chainVerified)&&this.verified?.summary.status==='unfinished';}
  private synchronized():boolean{return !!this.game&&!!this.transport?.connected&&this.peerSeen===hash(this.game);}
  private playable():boolean{return this.canSign()&&(this.manual||this.synchronized());}
  async act(kind:'move'|'resignation',value=''):Promise<void>{
    check(this.canSign()&&this.game&&this.actor,'Resume your game and verify its opening before signing');check(kind==='resignation'||this.playable(),'Connect and synchronize devices, or choose Use manual exchange before moving');const token=this.generation;
    ...
    this.notice(this.verified?.summary.status==='finished'?'Finished result saved. It can be published without opponent approval.':this.transport?.connected?'Signed action saved locally. Waiting for the peer to return matching history.':'Signed action saved on this device only. Copy and send the message to your opponent.');
  ...
  private async connection():Promise<PeerTransport>{
    ...
    this.manual=false;if(this.transport)return this.transport;const key=await this.journal.key(this.game.opening[this.actor].key);check(key,'Private game key unavailable');const opening=this.game.opening,token=this.generation;
    this.transport=new PeerTransport(opening,this.actor,key,async text=>{
      ...
      const before=this.game?hash(this.game):null,incoming:Game={opening:a.opening,line:a.line,branches:a.branches},merged=await this.journal.receive(incoming);if(token!==this.generation)return;
      this.peerSeen=hash(incoming);await this.useGame(merged,this.chainVerified,false,true);
      ...
      // Return new durable history once as a receipt; equal receipts do not loop.
      if(hash(merged)!==before||hash(merged)!==hash(incoming))await this.sendCurrent();
    },text=>{if(token===this.generation){this.el('connection').textContent=text;if(!this.transport?.connected)this.peerSeen=null;this.render();}},()=>{if(token===this.generation)void this.sendCurrent();});return this.transport;
  }
  ...
  async loadRegistered():Promise<void>{
    ...
    const opening=await registry.confirmed(id),game=await this.journal.start(opening);await this.useGame(game,true,false);this.notice('Players and settings confirmed on chain. Next choose Connect devices or Use manual exchange. Registration does not deliver moves.');
  }
  ...
  private drawClock():void{this.tickClock();if(this.clock)this.clock.running=this.dialog.open&&!this.manual&&this.synchronized()&&this.canSign();if(!this.game?.opening.descriptor.clock.baseMs){this.el('clocks').textContent='Untimed';return;}const fmt=(ms:number)=>{const sec=Math.ceil(Math.max(0,ms)/1000);return `${Math.floor(sec/60)}:${String(sec%60).padStart(2,'0')}`;};this.el('clocks').textContent=`Advisory · White ${fmt(this.clock?.white??0)} · Black ${fmt(this.clock?.black??0)}. ${this.clock?.running?'Local estimate while connected':'Paused — waiting for synchronized connection; manual exchange has no running clock'}. Resets on reload. Expiration never awards a win.`;}
  private render():void{
    ...
    this.el('readiness').textContent=!this.game?'First create or load a game. Then connect devices or choose manual exchange.':this.readOnly?'Archive viewer. No live connection is needed.':this.verified?.summary.status!=='unfinished'?'Signed evidence saved. Completed results can be published without opponent approval.':this.manual?'Manual exchange: moves stay on this device until you copy and send them. Import your opponent’s full signed message to receive their move.':this.synchronized()?'Connected and synchronized. Moves are exchanged automatically while both game views stay open.':this.transport?.connected?'Connected; waiting for matching signed history from the other device. If it does not arrive, reconnect or use manual exchange.':'Devices are not connected. Registration alone does not send moves. Choose Connect devices or Use manual exchange to play.';
    ...
    for(const name of ['manual','connect-guide'])disabled(name,this.busy||!this.canSign());
    disabled('resign',this.busy||!this.canSign());disabled('export-message',this.busy||!this.game);disabled('public',this.busy||!this.game);disabled('backup',this.busy||!this.actor||this.readOnly);disabled('review',this.busy||this.verified?.summary.status!=='finished');disabled('verify-opening',this.busy||this.game?.opening.kind!=='chain');
