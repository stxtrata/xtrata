import { ReadQueue, fetchRead, cancelledRead, retryAfterMs } from './read-transport.js';
import type { ReadPriority } from './read-transport.js';
...
export interface EndpointOptions {
  network?: Network;
  /** Deadline per endpoint attempt, including response body. */
  timeoutMs?: number;
  /** Minimum spacing between new reads; injected transports can choose their own clock. */
...
export interface Endpoint {
  ...
  request(path: string, init?: RequestInit, priority?: ReadPriority): Promise<Response>;
  /**
...
export function makeEndpoint(options: EndpointOptions = {}): Endpoint {
  ...
  return {
     ...
     */
    async request(path: string, init?: RequestInit, priority: ReadPriority = 'auto'): Promise<Response> {
      // POST is allowed only for contract read-only calls. Never retry a broadcast.
      ...
      const pairs = !headers ? [] : Array.isArray(headers) ? headers :
        typeof (headers as Headers).entries === 'function' ? [...(headers as Headers).entries()] : Object.entries(headers);
      ...
      // Independent abort signals must never cancel another caller's read.
      const shared = init?.signal ? undefined : inFlight.get(key);
  ...
  async function sweepBases(path: string, init?: RequestInit): Promise<Response> {
    {
      ...
      for (let n = 0; n < bases.length; n++) {
        ...
        try {
          const response = await fetchRead(doFetch, `${base}${path}`, init, options.timeoutMs ?? 15_000);
          noteBudget(response);
          ...
          if (init?.signal?.aborted || (error as Error)?.name === 'AbortError') throw cancelledRead();
          lastError = error;
        }
      }
