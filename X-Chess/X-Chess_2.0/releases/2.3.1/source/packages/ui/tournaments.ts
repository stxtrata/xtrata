...
import { rememberedGame, rememberGame, rulesKeyOf, factsScope } from '../chain/game-facts.js';
import {
...
export async function loadTournament(id: number, deps: TournamentDeps): Promise<TournamentView> {
  ...
  for (const game of tournament.games) {
    ...
    const known = rememberedGame(game.id, factsScope(deps.chain));
...
export async function scoreTournament(
  ...
  for (const game of tournament.games) {
    ...
    if (options.only && !options.only.has(game.id)) {
      const held = rememberedGame(game.id, factsScope(deps.chain));
      if (held) {
        done++;
    ...
    const seen = rememberedGame(game.id, factsScope(deps.chain));
