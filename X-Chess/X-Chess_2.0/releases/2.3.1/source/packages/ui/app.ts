  ...
  private async guard<T>(what: string, run: () => Promise<T>): Promise<T | null> {
    ...
    try {
      ...
      if (what === 'computing ratings') {
        ...
        this.notice('leaderboardNote', 'warn', 'Verification stopped before every game could be read. The previous table has not been replaced. Retry Verify from scratch when the endpoint is available.');
      }
      return null;
    } finally {
