...
async function command(c:any):Promise<any>{
  ...
  if(c.op==='state')return {status:q('status').textContent,identity:q('identity').textContent,connection:q('connection').textContent,position:q('position').textContent,reviewDisabled:(q('review') as HTMLButtonElement).disabled,clocks:q('clocks').textContent,errors:(window as any).__peerErrors,external:(window as any).__peerExternal,overflow:document.documentElement.scrollWidth>innerWidth,boardSquares:q('board').querySelectorAll('[data-square]').length,boardHeight:q('board').getBoundingClientRect().height,squareHeight:q('board').querySelector('[data-square]')!.getBoundingClientRect().height};
  if(c.op==='move'){
    for(const square of [c.uci.slice(0,2),c.uci.slice(2,4)]){const el=q('board').querySelector<HTMLElement>(`[data-square="${square}"]`);if(!el)throw Error('No square '+square);el.click();await until(()=>!(q('demo') as HTMLButtonElement).disabled);}
