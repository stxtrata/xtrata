...
if(version!=='2.3.3'||manifest.build!==version||sha(await readFile('dist/xchess.html'))!==manifest.htmlSha256)throw Error('Wrong release bytes');
...
await writeFile(out+'/VALIDATION.json',JSON.stringify({version,artifactSha256:manifest.htmlSha256,bytes:manifest.bytes,xtrataChunks:manifest.xtrataChunks,packagedAt:new Date().toISOString(),reports,sourceRebuild:true,protocolsUnchangedFrom:'2.3.1',notes:['Suite counts overlap. Three live tests and the vector generator are skipped; optional deep perft is not required for unchanged engine code.','44 captured-runtime browser checks and 16 native peer checks use the exact release HTML.','Nine peer UI regressions cover registered readiness, manual e4/e5 handoff, saved-game recovery, input errors, clocks and disconnected winner publication; the prior ten display regressions also run.','No new mainnet transactions were needed. Real wallet-extension signing and final sealed-byte read-back remain post-inscription checks.','Clarinet check passed with 12 existing warnings.','Native tests use desktop Chrome, not actual iOS Xverse. Real phone rendering and two-device handoff still need a device check.']},null,2)+'\n');
