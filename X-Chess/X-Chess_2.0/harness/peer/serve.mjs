...
createServer(async(req,res)=>{
  ...
  try{
    ...
    if(u.pathname==='/mobile')return send('<!doctype html><meta name=viewport content="width=device-width,initial-scale=1"><style>body{margin:0}iframe{display:block;width:390px;height:100vh;border:0}</style><iframe title="Mobile peer test" src="/app?side='+encodeURIComponent(u.searchParams.get('side')||'visual')+'"></iframe>','text/html');
    if(u.pathname==='/app'){
...
}).listen(Number(process.env.PORT||4343),'127.0.0.1',()=>console.log('Peer browser fixture http://127.0.0.1:4343/app?side=white'));
