;; xchess-core-v3 - local development only; never deployed.
;;
...
(define-constant PROTOCOL "xchess-core-v3")

;; Immutable descriptors are stored at opening, alongside the V2 commitment.
;; The options are opaque, versioned bytes interpreted by clients. They must
;; never become authoritative turn, legality, result, or payout claims.
(define-map Descriptors uint {
  rules: (buff 2048),
  options-version: uint,
  options: (buff 1024),
  match-ref: (string-ascii 128)
})
(define-constant ERR-BAD-DESCRIPTOR (err u120))
(define-constant ERR-STALE-QUOTE (err u121))

(define-read-only (get-capabilities)
  { log-format: u1, descriptor-format: u1, max-rules-bytes: u2048,
    max-options-bytes: u1024, move-fee: u0, expiry-stops-rebates: true })

(define-read-only (get-descriptor (game uint)) (map-get? Descriptors game))

(define-read-only (get-protocol-v3)
  { protocol: u3, opening-fee: (var-get open-fee), move-fee: u0, treasury: SELF })

(define-read-only (get-v3-game (game uint))
  (match (map-get? Descriptors game) descriptor
    (match (map-get? Games game) metadata
      (some { rules: (get rules descriptor),
        rules-hash: (sha256 (get rules descriptor)), height: (get opened-at metadata),
        options-version: (get options-version descriptor), options: (get options descriptor) })
      none)
    none))

(define-read-only (get-count (game uint))
  (get next-seq (unwrap-panic (map-get? Games game))))

(define-read-only (get-funding (game uint) (who principal))
  (match (map-get? Sponsorships {game: game, who: who}) row
    (some {remaining: (get rebates-left row), rebate: (get rebate row),
      expiry: (get expiry row), settled: (get settled row)}) none))

(define-read-only (quote-rebate (game uint) (who principal))
  (match (map-get? Sponsorships {game: game, who: who}) row
    (if (and (not (get settled row)) (< stacks-block-height (get expiry row))
      (> (get rebates-left row) u0) (>= (get reserved row) (get rebate row)))
      (get rebate row) u0) u0))

(define-constant V3-OFFSETS (list u0 u1 u2 u3 u4 u5 u6 u7 u8 u9 u10 u11 u12 u13 u14 u15 u16 u17 u18 u19 u20 u21 u22 u23 u24 u25 u26 u27 u28 u29 u30 u31))
(define-private (v3-entry-step (offset uint) (acc {game: uint, start: uint,
  out: (list 32 (optional {seq: uint, value: (string-ascii 5), sender: principal, height: uint}))}))
  (let ((seq (+ (get start acc) offset)))
    (merge acc {out: (unwrap-panic (as-max-len? (append (get out acc)
      (match (map-get? Entries {game: (get game acc), seq: seq}) entry
        (some (merge entry {seq: seq})) none)) u32))})))
(define-read-only (get-entries-page (game uint) (start uint))
  (get out (fold v3-entry-step V3-OFFSETS {game: game, start: start, out: (list)})))

(define-private (v3-game-step (offset uint) (acc {start: uint,
  out: (list 32 {id: uint, metadata: (optional {rules: (buff 2048), rules-hash: (buff 32), height: uint, options-version: uint, options: (buff 1024)})})}))
  (let ((id (+ u1 (get start acc) offset)))
    (merge acc {out: (unwrap-panic (as-max-len? (append (get out acc)
      {id: id, metadata: (get-v3-game id)}) u32))})))
(define-read-only (get-games-page (start uint))
  (get out (fold v3-game-step V3-OFFSETS {start: start, out: (list)})))

(define-private (custom-funding-cost
    (row {beneficiary: principal, bootstrap: uint, rebate: uint, count: uint, expiry: uint})
    (acc uint))
  (+ acc (get bootstrap row) (* (get rebate row) (get count row))))

(define-private (custom-funding
    (row {beneficiary: principal, bootstrap: uint, rebate: uint, count: uint, expiry: uint})
    (acc (response uint uint)))
  (let ((game (try! acc)) (who (get beneficiary row))
        (liability (* (get rebate row) (get count row))))
    (asserts! (and (<= (get bootstrap row) BOOTSTRAP-CEILING)
      (> (get rebate row) u0) (<= (get rebate row) REBATE-CEILING)
      (> (get count row) u0) (<= (get count row) u65536)
      (> (get expiry row) stacks-block-height)) ERR-ABOVE-CEILING)
    (asserts! (is-none (map-get? Sponsorships {game: game, who: who})) ERR-ALREADY-SPONSORED)
    (map-insert Sponsorships {game: game, who: who}
      {rebates-left: (get count row), rebate: (get rebate row), reserved: liability,
       expiry: (get expiry row), settled: false, funded-by: tx-sender})
    (reserve liability)
    (try! (pay-out (get bootstrap row) who))
    (ok game)))

;; Canonical rules carry ranked consent. The ranked index remains a hint.
(define-public (open-v3-game (rules (buff 2048)) (expected-open-fee uint)
    (sponsors (list 2 {beneficiary: principal, bootstrap: uint, rebate: uint, count: uint, expiry: uint})))
  (begin
    (asserts! (> (len rules) u0) ERR-BAD-DESCRIPTOR)
    (asserts! (is-eq expected-open-fee (var-get open-fee)) ERR-STALE-QUOTE)
    (try! (collect (+ expected-open-fee (fold custom-funding-cost sponsors u0))))
    (let ((id (register (some (descriptor-hash rules u1 0x "")) false)))
      (map-insert Descriptors id {rules: rules, options-version: u1, options: 0x, match-ref: ""})
      (try! (fold custom-funding sponsors (ok id)))
      (ok id))))

(define-public (top-up (game uint) (who principal) (count uint) (expected-rebate uint))
  (let ((row (unwrap! (map-get? Sponsorships {game: game, who: who}) ERR-NO-SPONSORSHIP))
        (amount (* count (get rebate row))))
    (asserts! (and (not (get settled row)) (< stacks-block-height (get expiry row))) ERR-NOT-EXPIRED)
    (asserts! (and (> count u0) (<= (+ count (get rebates-left row)) u65536)) ERR-ABOVE-CEILING)
    (asserts! (is-eq expected-rebate (get rebate row)) ERR-STALE-QUOTE)
    (try! (collect amount))
    (map-set Sponsorships {game: game, who: who} (merge row
      {rebates-left: (+ count (get rebates-left row)), reserved: (+ amount (get reserved row))}))
    (reserve amount)
    (ok true)))
(define-public (settle (game uint) (who principal)) (settle-sponsorship game who))

;; Includes every field in the commitment, including extension version and
;; match identity. Repeated identical descriptors still receive distinct IDs.
(define-read-only (descriptor-hash
    (rules (buff 2048)) (options-version uint)
    (options (buff 1024)) (match-ref (string-ascii 128)))
  (sha256 (unwrap-panic (to-consensus-buff? {
    rules: rules, options-version: options-version,
    options: options, match-ref: match-ref }))))

(define-public (open-described-game
    (rules (buff 2048)) (options-version uint)
    (options (buff 1024)) (match-ref (string-ascii 128))
    (ranked bool) (expected-open-fee uint))
  (begin
    (asserts! (and (> (len rules) u0) (> options-version u0)) ERR-BAD-DESCRIPTOR)
    (asserts! (is-eq expected-open-fee (var-get open-fee)) ERR-STALE-QUOTE)
    (try! (collect expected-open-fee))
    (let ((id (register (some (descriptor-hash rules options-version options match-ref)) ranked)))
      (map-insert Descriptors id {
        rules: rules, options-version: options-version,
        options: options, match-ref: match-ref })
      (ok id))))

;; Third parties may fund an already opened described (or V2-format) game.
;; Requiring all quote fields prevents owner price changes before inclusion
;; from changing either the cost or the funding promise.
(define-public (sponsor-game
    (game uint) (who principal) (expected-bootstrap uint)
    (expected-rebate uint) (expected-count uint)
    (expected-margin uint) (expected-expiry-blocks uint))
  (begin
    (asserts! (is-some (map-get? Games game)) ERR-NO-GAME)
    (asserts! (and
      (is-eq expected-bootstrap (var-get bootstrap-amount))
      (is-eq expected-rebate (var-get rebate-amount))
      (is-eq expected-count (var-get rebate-count))
      (is-eq expected-margin (var-get sponsor-margin))
      (is-eq expected-expiry-blocks (var-get expiry-blocks))) ERR-STALE-QUOTE)
    (try! (collect (get total (get-sponsor-price))))
    (try! (fund game who))
    (ok true)))
...
(define-private (reserve (amount uint))
 ...
 ;; Reserve reconciliation must abort rather than understate liabilities.
(define-private (release (amount uint))
  (begin
    (asserts! (<= amount (var-get total-reserved)) ERR-WOULD-UNDERFUND)
    (var-set total-reserved (- (var-get total-reserved) amount))
    (ok true)
  )
)
...
(define-public (top-up-sponsorship (game uint) (who principal))
  (let (
      ...
      (price (get-sponsor-price))
      (liability (* (get rebate existing) (var-get rebate-count)))
    )
    (asserts! (is-some (map-get? Games game)) ERR-NO-GAME)
    ...
    sponsorship
      (let ((amount (get rebate sponsorship)))
        (if (or
              ...
              (>= stacks-block-height (get expiry sponsorship))
