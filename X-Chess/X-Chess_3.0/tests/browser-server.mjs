...
import {uintCV, someCV, noneCV, tupleCV, trueCV, standardPrincipalCV, serializeCV} from '../../X-Chess_2.0/node_modules/@stacks/transactions/dist/index.js';
const root = resolve(fileURLToPath(new URL('../../X-Chess_2.0', import.meta.url)));
let artifact = await readFile(resolve(root, '../X-Chess_3.0/dist/xchess.html'), 'utf8');
let sha256 = createHash('sha256').update(artifact).digest('hex');
...
const server = createServer(async (req,res) => {
  artifact = await readFile(resolve(root, '../X-Chess_3.0/dist/xchess.html'), 'utf8');
  ...
  try {
    ...
    if (url.pathname === '/v3-tests') return send('<!doctype html><meta charset="utf-8"><title>V3 browser tests</title><pre id="results">Running…</pre><div id="scenarios"></div><script src="/v3-tests.js"></script>');
    if (url.pathname === '/v3-tests.js') return send(await readFile(resolve(root,'../X-Chess_3.0/tests/browser-v3.js'),'utf8'),'text/javascript');
    ...
    if (url.pathname === '/report' && req.method === 'POST') {
      ...
      await writeFile(resolve(root, url.searchParams.has('arena') ? '../X-Chess_3.0/browser-v3-report.json' : '../X-Chess_3.0/browser-v2-report.json'), JSON.stringify(lastReport,null,2)+'\n');
    ...
    if (url.pathname === '/scenario') {
      ...
      const prelude=`<script>localStorage.clear();{const u=new URL(location.href);u.searchParams.set('xchess-view','${url.searchParams.has('arena') ? 'v3' : 'v2'}');history.replaceState(null,'',u);}window.__XCHESS_API__='/fixture-api';window.__testCalls=[];window.__testErrors=[];addEventListener('error',e=>window.__testErrors.push(e.message));addEventListener('unhandledrejection',e=>window.__testErrors.push(String(e.reason)));if(${mode!=='framed'})window.StacksProvider={request:async(method)=>{window.__testCalls.push(method);if(${refuse})throw Object.assign(new Error('User rejected connection'),{code:4001});return {addresses:[{symbol:'STX',address:'${url.searchParams.has('arena') ? 'STC5KHM41H6WHAST7MWWDD807YSPRQKJ68T330BQ' : address}'}]};}};</script>`;
...
server.listen(Number(process.env.PORT || 4349),'127.0.0.1',()=>console.log('Browser regression: http://127.0.0.1:'+server.address().port+' (artifact '+sha256+')'));
