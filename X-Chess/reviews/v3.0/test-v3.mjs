...
const source = process.argv[2] || '/Users/melophonic/Downloads/x-chess-3.0.html';
...
function boot({instrument=false, url='https://example.test/i/9999', storage}={}) {
  ...
  const input=instrument ? html.replace('window.XChessBoot=', 'window.__audit={Chess:gn,replay:Ta,rules:hy,reader:jm,parseLink:lT,connect:dT,providers:Qc,exportBundle:uT,importBundle:cT,demo:bT};window.XChessBoot=') : html;
  const dom=new JSDOM(input,{url,runScripts:'dangerously',pretendToBeVisual:true,virtualConsole:new VirtualConsole(),beforeParse(w){
    ...
    if(storage)Object.defineProperty(w,'sessionStorage',{get(){throw Error('storage denied');}});
...
await test('UI: flip reverses board',async()=>{await click('Flip board');assert.equal(d.querySelector('[data-square]').dataset.square,'h1');await click('Flip board');});
...
await test('UI: replay refuses new moves',async()=>{await click('Starting position');await square('e2');await square('e4');assert.ok(!d.querySelector('.move-preview'),'Unexpected move preview');assert.match(d.body.textContent,/Return to the live position/);await click('Live position');});
...
ui.dom.window.close();
const isolated=boot({instrument:true});await pause();const api=isolated.w.__audit;assert.ok(api,'Instrumentation point missing');
...
await test('Compatibility: accepts v2.2 contract protocol response',async()=>{const reader=new api.reader({tip:async()=>({hash:'abc',height:1}),read:async(_,fn)=>{if(fn==='get-protocol')return 'xchess-core-v1';throw Error('Unexpected '+fn);}});await reader.game({network:'mainnet',contract:'SP3JNSEXAZP4BDSHV0DN3M8R3P0MY0EEBQQZX743X.xchess-core-v1-canary',gameId:47});});
await test('Compatibility: accepts legacy #game=47 shared link',()=>assert.equal(api.parseLink('#game=47').id.gameId,47));
...
await test('UI: blocked session storage does not crash boot',async()=>{const b=boot({storage:true});await pause();assert.equal(b.d.querySelectorAll('[data-square]').length,64);assert.deepEqual(b.errors,[]);b.dom.window.close();});
await test('Artifact: fits existing 32-chunk budget',()=>assert.ok(Math.ceil(Buffer.byteLength(html)/16384)<=32));
...
writeFileSync(fileURLToPath(new URL('./v3-results.json',import.meta.url)),JSON.stringify(report,null,2)+'\n');
