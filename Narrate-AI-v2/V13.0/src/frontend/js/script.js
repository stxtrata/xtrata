...
function updateChunkUI(id,st){
    ...
    const icon=el.querySelector('.generated-icon'); if(icon) icon.style.opacity = st==='done'?1:0;
            ...
            <div class="chapter-header">
                <div class="ch-row">
                    <div style="display:flex;align-items:center;gap:6px;flex:1;overflow:hidden">
                        ...
                        <span class="ch-title" title="${escapeHtml(ch.title)}" onclick="displayChapter(${idx})" style="cursor:pointer">${escapeHtml(ch.title)}</span>
                    </div>
                    <div class="ch-meta">
...
// V13: matches the server's ElevenLabs queue limit so the client keeps the pipe full
// without overrunning it. Chunks within a chapter are independent files, so generating
// them with bounded concurrency (instead of strictly one-at-a-time) ~halves wall time.
...
async function generateChapterChunksParallel(ch, chapterIndex, onProgress){
    ...
    await runPool(ch.chunks, GEN_CONCURRENCY, async (ck, j) => {
...
async function retryFailedChunks(){
    ...
    await runPool(failed, GEN_CONCURRENCY, async ({i,j}) => {
        if(STATE.halt) return;
