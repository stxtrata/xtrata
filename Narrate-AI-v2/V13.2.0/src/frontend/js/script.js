...
const STATE = {
    ...
    voices: [],
    project: {
        id: (typeof generateProjectId === 'function' ? generateProjectId() : 'proj_' + Date.now().toString(36)),
        ...
        silenceChapter: 1.0,
        notes: '',
        customTitle: '',   // V13: user-set project name (from header field)
        sourceHash: '',    // V13: hash of manuscript at last analyze (detects new book)
        analyzedOnce: false
    },
    projectMeta: {
        title: 'Untitled',
...
// V13: expose state on window for console debugging and the browser smoke-test harness.
...
const ChapterVerifier = {
    ...
    // French compounds 20-69: "vingt", "vingt et un(e)" (21), "vingt-deux" (22) ... "soixante-neuf".
    // (Bare "vingt" used to greedily match 20 and falsely flag "Vingt et Un" / chapter 21.)
    parseFrenchCompound(clean) {
        ...
        const m = clean.match(/^(vingt|trente|quarante|cinquante|soixante)(?:[-\s]+et[-\s]+(une?)|[-\s]+(deux|trois|quatre|cinq|six|sept|huit|neuf))?(?=$|[\s:.!?;,])/i);
        if (!m) return null;
        const base = tens[m[1].toLowerCase()];
        if (m[2]) return base + 1;                       // "... et un / et une"
        if (m[3]) return base + ones[m[3].toLowerCase()]; // hyphenated "...-deux"
        return base;                                      // bare tens
    },
    parse(title) {
        ...
        // 3b. French compounds (e.g., "vingt et un" = 21, "trente-deux" = 32)
        const french = this.parseFrenchCompound(clean);
...
if (typeof window !== 'undefined') window.ChapterVerifier = ChapterVerifier; // debug/tests
...
function buildProjectPayload(){
    ...
    // V13: the header project-name field is the primary source of truth, falling back to
    // the Projects-modal field and any previously stored custom title.
    const headerName=(document.getElementById('header-project-name')?.value||'').trim();
    const modalName=(document.getElementById('project-title-input')?.value||'').trim();
    const customTitle = headerName || modalName || (STATE.project.customTitle||'').trim();
    STATE.project.customTitle = customTitle;
    ...
    STATE.project.genConcurrency=getGenConcurrency();
    ...
    return {
        ...
        sessionCost: STATE.sessionCost,   // N8: persist spend so partial reloads keep the running total
...
async function connectElevenLabs(silent=false){
    ...
    try{
        ...
        const cbtn=document.getElementById('btn-connect-el');
        cbtn.innerText="✓ Connected";cbtn.classList.add('connected');  // N7: green when connected
...
function displayChapter(idx){
    ...
    dom.manuscript.scrollTop=0;
    STATE.editingChunkId=null;
    if(dom.btnSaveText) dom.btnSaveText.style.display='none';
}

// N4 (revised): clicking anywhere on a chapter/title block selects it (shows its text);
// clicking the same block again toggles back to the full book text. Replaces the top button.
function toggleChapterSelection(idx, e){
    if(e){ if(e.target && e.target.closest && e.target.closest('button')) return; } // ignore control buttons
    if(STATE.selectedChapterIdx === idx){
        STATE.selectedChapterIdx = null;
        showFullBookText();
    } else {
        STATE.selectedChapterIdx = idx;
        displayChapter(idx);
        LOG.add(`Viewing: ${STATE.chapters[idx] ? STATE.chapters[idx].title : 'section'} (click again for full book).`);
    }
    highlightSelectedChapter();
}
function highlightSelectedChapter(){
    if(!dom.timeline) return;
    dom.timeline.querySelectorAll('.chapter-card.selected').forEach(el=>el.classList.remove('selected'));
    if(STATE.selectedChapterIdx != null){
        const card = dom.timeline.querySelector(`[data-chapter-card="${STATE.selectedChapterIdx}"]`);
        if(card) card.classList.add('selected');
    }
}
function showFullBookText(){
    dom.manuscript.value = STATE.fullManuscript || STATE.chapters.map(ch=>(ch.chunks||[]).map(c=>c.text).join('\n\n')).join('\n\n');
    dom.manuscript.scrollTop=0;
    STATE.editingChunkId=null;
    STATE.selectedChapterIdx=null;
    if(dom.btnSaveText) dom.btnSaveText.style.display='none';
...
function getEffectiveProjectName(){
    const header=(document.getElementById('header-project-name')?.value||'').trim();
    return header
        || (STATE.project.customTitle||'').trim()
        || STATE.projectMeta.displayTitle || STATE.projectMeta.title || 'Untitled';
}
// Reflect the current project name into the header field (unless the user is editing it).
function syncHeaderProjectName(){
    const el=document.getElementById('header-project-name');
    if(!el || document.activeElement===el) return;
    const name=(STATE.project.customTitle||'').trim()
        || (STATE.projectMeta.displayTitle && STATE.projectMeta.displayTitle!=='Untitled' ? STATE.projectMeta.displayTitle : '');
    el.value=name;
}
function getGenerationDisplayTitle(){
    return getEffectiveProjectName();
}
// N3: download links carry a readable filename derived from the book title
// (the same title shown in the Confirm Generation modal), e.g. "The Great Adventure.mp3".
function bookDownloadLink(url, label){
    const fname = NarrateCore.toReadableFilename(getEffectiveProjectName(), 'mp3');
    return `<a href="${url}" download="${escapeHtml(fname)}" target="_blank" style="color:#fff;text-decoration:underline">${label||'Download'} (${escapeHtml(fname)})</a>`;
}
// Issue 2: classify each section so we report counts accurately. Falls back to inferring
// the kind from the title for projects saved before sections were tagged.
function getChapterKind(ch){
    if(ch && ch.kind) return ch.kind;
    const t = (ch && ch.title) || '';
    if(t === 'Titles') return 'titles';
    const c = (typeof NarrateCore!=='undefined') ? NarrateCore.classifyHeading(t) : {numbered:true,kind:'chapter'};
    return c.numbered ? 'chapter' : c.kind;
}
const SECTION_LABELS = { titles:'Titles', prologue:'Prologue', epilogue:'Epilogue', preface:'Preface', introduction:'Introduction', foreword:'Foreword', prelude:'Prelude', afterword:'Afterword', postscript:'Postscript' };
function summarizeChapterCounts(){
    let chapters = 0; const extras = [];
    (STATE.chapters||[]).forEach(ch=>{
        const k = getChapterKind(ch);
        if(k === 'chapter') chapters++;
        else extras.push(SECTION_LABELS[k] || (ch.title||'Section'));
    });
    return { chapters, extras };
}
function getGenerationChapterLabel(){
    const c = summarizeChapterCounts();
    const extras = c.extras.length ? ` + ${c.extras.join(', ')}` : '';
    return `${c.chapters} chapter${c.chapters===1?'':'s'}${extras}`;
}
...
async function processChunkGeneration(chunk,chIdx,ckIdx,retry=0,force=false){
    ...
    chunk.status='processing';updateChunkUI(chunk.id,'processing');refreshChapterStatus(chIdx);
    try{
        ...
        chunk.duration=res.duration;chunk.audioUrl=res.audioUrl;chunk.filename=res.filename;chunk.status='done';updateChunkUI(chunk.id,'done');refreshChapterStatus(chIdx);
        ...
        chunk.status='error';updateChunkUI(chunk.id,'error');refreshChapterStatus(chIdx);LOG.add(e.message,'error');return false;
    }
}
function updateChunkUI(id,st){
    const el=document.getElementById(`chunk-${id}`);
    if(!el) return;
    el.className=`chunk-item status-${st}`;
    el.setAttribute('data-status', st);
    // N8: per-region tick reflects backend state live — orange (pulsing) while processing,
    // green when the audio file is written, red on error.
    const icon=el.querySelector('.generated-icon');
    if(icon){
        icon.classList.remove('icon-done','icon-processing','icon-error');
        if(st==='done'){ icon.textContent='✓'; icon.classList.add('icon-done'); icon.style.opacity=1; }
        else if(st==='processing'){ icon.textContent='✓'; icon.classList.add('icon-processing'); icon.style.opacity=1; }
        else if(st==='error'){ icon.textContent='✕'; icon.classList.add('icon-error'); icon.style.opacity=1; }
        else { icon.textContent='✓'; icon.style.opacity=0; }
    }
    if(st==='error') updateRetryButtonVisibility();
}

// N8: live per-chapter status pip (pulsing orange while any region processes; green when all done).
function refreshChapterStatus(idx){
    const el=document.getElementById(`chapter-status-${idx}`); if(!el) return;
    const ch=STATE.chapters[idx];
    el.className='chapter-status';
    if(!ch||!ch.chunks||!ch.chunks.length){ el.textContent=''; return; }
    const anyProcessing=ch.chunks.some(c=>c.status==='processing');
    const anyError=ch.chunks.some(c=>c.status==='error');
    const allDone=ch.chunks.every(c=>c.status==='done');
    if(anyProcessing){ el.textContent='●'; el.classList.add('cs-processing'); }
    else if(allDone){ el.textContent='✓'; el.classList.add('cs-done'); }
    else if(anyError){ el.textContent='!'; el.classList.add('cs-error'); }
    else { el.textContent=''; }
}
function refreshAllChapterStatuses(){ for(let i=0;i<STATE.chapters.length;i++) refreshChapterStatus(i); }
...
async function generateChapter(idx,e){
    ...
    document.getElementById('btn-confirm-start').onclick=async()=>{closeSafetyModal();STATE.halt=false;document.getElementById('btn-halt').style.display='inline-block';
        const results=await generateChapterChunksParallel(ch, idx);
        const files=results.filter(Boolean); const ok=files.length;
...
// Escape user text before inserting into innerHTML (manuscript can contain < & " etc.)
function escapeHtml(s){return String(s==null?'':s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}

// Build a voiceId -> voice lookup ONCE per render instead of STATE.voices.find() per chunk.
function buildVoiceMap(){const m=new Map();(STATE.voices||[]).forEach(v=>m.set(v.id,v));return m;}

// Full STRUCTURAL render. Used when chapters/chunks/collapse change - NOT on every
// playback tick. Playback highlighting is handled incrementally by updatePlaybackHighlight().
function renderTimeline() {
    ...
    dom.timeline.innerHTML = STATE.chapters.map((ch, idx) => {
        ...
        const allDone = ch.chunks.length > 0 && ch.chunks.every(ck => ck.status === 'done');
        const collapsed = ch.collapsed;

        const chunksHtml = !collapsed ? ch.chunks.map((ck, cIdx) => {
            const st = `status-${ck.status}`;
            const bg = cIdx % 2 === 0 ? 'background:rgba(255,255,255,0.02);' : '';
            const chunkVoice = voiceMap.get(ck.voiceId || STATE.project.voiceIds[ck.voiceIndex] || STATE.project.voiceIds[0]);
            const vClass = getVoiceColorClassByVoice(chunkVoice);

            return `<div id="chunk-${ck.id}" class="chunk-item ${st}" data-status="${ck.status}" data-cidx="${cIdx}" onclick="playSingleChunk('${ck.id}')" style="${bg}" role="button">
                <div style="display:flex;align-items:center;gap:10px;width:100%">
                    <span class="chunk-idx" style="font-family:var(--font-mono);font-size:0.8rem;color:var(--text-dim);width:20px;text-align:center">${cIdx+1}</span>
                    <span class="voice-tag ${vClass}" style="margin:0;width:80px;text-align:center">${escapeHtml(ck.voiceName||'Voice')}</span>
                    <span class="chunk-content" style="flex:1">${escapeHtml(ck.text.substring(0,60))}...</span>
        ...
        return `<div class="chapter-card${idx===STATE.selectedChapterIdx?' selected':''}" data-chapter-card="${idx}">
            <div class="chapter-header" onclick="toggleChapterSelection(${idx},event)" title="Click to view this section's text (click again for the full book)" style="cursor:pointer">
                <div class="ch-row">
                    <div style="display:flex;align-items:center;gap:6px;flex:1;overflow:hidden">
                        ...
                        <span class="ch-title" title="${escapeHtml(ch.title)}">${escapeHtml(ch.title)}</span>
                    </div>
                    <div class="ch-meta">
                ...
                <div class="ch-row" style="margin-top:6px;">
                    <div style="display:flex;gap:5px;align-items:center">
                        ...
                        <span class="chapter-status ${allDone?'cs-done':''}" id="chapter-status-${idx}">${allDone?'✓':''}</span>
                    </div>
                    <div style="display:flex;gap:5px;">
                        ${ch.chunks.length ? `<button class="sm-btn" data-playbtn="${idx}" onclick="playChapter(${idx},event)">▶</button>` : ''}
    ...
    updateRetryButtonVisibility();
    refreshAllChapterStatuses();
    highlightSelectedChapter();
}

// Lightweight, incremental playback highlight. Toggles classes on the two affected
// nodes instead of rebuilding the whole timeline - this is what keeps playback smooth
// and preserves scroll position on large books.
function updatePlaybackHighlight(){
    const tl = dom.timeline; if(!tl) return;
    tl.querySelectorAll('.chunk-item.status-playing').forEach(el=>{
        const st = el.getAttribute('data-status') || 'pending';
        el.className = `chunk-item status-${st}`;
        const ix = el.querySelector('.chunk-idx');
        if(ix){ ix.textContent = String((parseInt(el.getAttribute('data-cidx'),10)||0)+1); ix.style.color='var(--text-dim)'; }
        const vt = el.querySelector('.voice-tag'); if(vt) vt.classList.remove('playing');
    });
...
// Show the "Retry Failed" control only when at least one chunk errored.
function updateRetryButtonVisibility(){
    const btn = document.getElementById('btn-retry-failed'); if(!btn) return;
    const hasFailed = STATE.chapters.some(ch => (ch.chunks||[]).some(c => c.status === 'error'));
...
// V13: sanitizeChunkSize() and splitTextIntoChunks() now live in js/core.js
// (single source of truth, shared with the Node test suite). core.js loads first and
// exposes them as globals, so existing call-sites below are unchanged.
...
document.getElementById('btn-analyze').addEventListener('click', () => {
    ...
    STATE.fullManuscript = raw; // N4: remember the full book text for the "Full Book" view
    ...
    // Iterate
    // N1: chapterNumber counts REAL numbered chapters only; headingsSeen tracks any heading
    // (so content before the first heading is still "Titles"). Front/back matter no longer
    // bumps the chapter number.
    let chapterNumber = 0;
    let headingsSeen = 0;
    let currentSection = { kind: 'chapter', numbered: true, label: null };
    // N5: whether to fold the heading into the spoken audio (toggle, default on).
    const speakHeadingsEl = document.getElementById('speak-headings');
    const speakHeadings = speakHeadingsEl ? speakHeadingsEl.checked : true;
    STATE.project.speakHeadings = speakHeadings;
    for(let i=0; i<parts.length; i++) {
        ...
        if(pClean.match(chapterRegex)) {
            ...
            headingsSeen++;
            currentSection = NarrateCore.classifyHeading(currentTitle);

            if (currentSection.numbered) {
                chapterNumber++;
                const detectedNum = ChapterVerifier.parse(currentTitle);
                if (detectedNum !== null && detectedNum !== chapterNumber) {
                    anomalies.push(`Mismatch: Chapter ${chapterNumber} titled "${currentTitle}" (Parsed: ${detectedNum})`);
                }
            }

            // Format header for audio (N5: only when the toggle is on).
            if (speakHeadings) {
             ...
             if(chapterChunks.length > 0) {
                ...
                let chapterKind; // N1/issue-2: tag each section so counts are accurate
                if (headingsSeen === 0) {
                    finalChapterTitle = "Titles";
                    chapterKind = 'titles';
                } else if (!currentSection.numbered) {
                    // N1: front/back matter keeps its own name (+ optional subtitle), no number.
                    const sub = currentTitle
                        .replace(new RegExp('^' + currentSection.label + '\\s*[:\\-–—]?\\s*', 'i'), '')
                        .trim();
                    finalChapterTitle = (sub && sub.toLowerCase() !== currentSection.label.toLowerCase())
                        ? `${currentSection.label}: ${sub}`
                        : currentSection.label;
                    chapterKind = currentSection.kind;
                } else {
                    ...
                    finalChapterTitle = `Chapter ${chapterNumber}${cleanHeading ? ': ' + cleanHeading : ''}`;
                    chapterKind = 'chapter';
                }
                 ...
                 summary.push({
                     index: currentSection.numbered ? chapterNumber : 0,
                     title: finalChapterTitle,
    ...
    STATE.project.sourceHash = srcHash;
    STATE.project.analyzedOnce = true;
    ...
    // Issue 2: type-aware section counts (chapters vs Titles/Prologue/Epilogue/...).
    const counts = summarizeChapterCounts();
    const label = `${counts.chapters}${counts.extras.length ? ' +' + counts.extras.join(',') : ''}`;
    ...
    // N2: auto-set the session budget to the estimated cost rounded UP to the next dollar.
    const budgetEl = document.getElementById('budget-limit');
    if (budgetEl) {
        const rounded = NarrateCore.roundUpToDollar(estCost);
        budgetEl.value = rounded.toFixed(2);
        LOG.add(`Session budget set to $${rounded.toFixed(2)} (rounded up from est. $${estCost.toFixed(2)}).`);
    }
...
async function rebuildBookFromExistingAudio(){
    ...
    try{
        ...
        renderTimeline();
        LOG.add(`Book rebuilt from existing audio (${rebuiltChapters} chapters). ${bookDownloadLink(res.url)}`,'success');
...
// V13.1: parallel-request count is user-configurable (default 10) up to the plan ceiling.
// Chunks within a chapter are independent files merged back in index order, so higher
// concurrency only speeds things up — it never reorders or corrupts the audio.
const MAX_CONCURRENCY = 15; // ElevenLabs Scale/Business ceiling
function getGenConcurrency(){
    const el = document.getElementById('gen-concurrency');
    return NarrateCore.clampConcurrency(el ? el.value : 2, MAX_CONCURRENCY);
}

// Run async `worker(item, index)` over `items` with at most `concurrency` in flight.
...
function countAllChunks(){return STATE.chapters.reduce((a,ch)=>a+(ch.chunks?ch.chunks.length:0),0);}

// Generate one chapter's chunks with bounded concurrency, returning filenames IN ORDER
// (nulls for missing/failed are dropped by the caller) so the merge stays correctly sequenced.
async function generateChapterChunksParallel(ch, chapterIndex, onProgress){
    const results = new Array(ch.chunks.length).fill(null);
    await runPool(ch.chunks, getGenConcurrency(), async (ck, j) => {
        if(STATE.halt) return;
        if(ck.status==='done' && ck.audioUrl){
            results[j] = ck.filename || ck.audioUrl.split('/').pop();
            if(onProgress) onProgress();
            return;
        }
        const ok = await processChunkGeneration(ck, chapterIndex, j);
        if(ok && ck.filename) results[j] = ck.filename;
        if(onProgress) onProgress();
    });
    return results;
}
...
async function startFullBookGeneration(){
    ...
    const total = countAllChunks();
    let done = 0;
    dom.masterProgress.value = 0;
    LOG.add(`Generating ${total} segments at ${getGenConcurrency()} parallel request${getGenConcurrency()===1?'':'s'}.`);
    const tick = () => { done++; if(total>0) dom.masterProgress.value = (done/total)*100; dom.btnGenerate.innerText = `Generating ${done}/${total}...`; };

    for(let i=0;i<STATE.chapters.length;i++){
        if(STATE.halt)break;
        const ch=STATE.chapters[i];
        const results = await generateChapterChunksParallel(ch, i, tick);
    ...
    if(!STATE.halt){
        try{
            ...
            const silenceChapter=parseFloat(dom.silenceChapter.value)||0;
            const res=await APIService.mergeBook(STATE.project.id, silenceChapter);
            LOG.add(`Full Book Generated! ${bookDownloadLink(res.url)}`,'success');
...
// V13: regenerate only the segments that previously errored, then re-merge affected
// chapters. Surfaced via the "Retry Failed" button in the timeline header.
async function retryFailedChunks(){
    const failed = [];
    STATE.chapters.forEach((ch,i)=>(ch.chunks||[]).forEach((ck,j)=>{ if(ck.status==='error') failed.push({i,j}); }));
    if(!failed.length){ LOG.add("No failed segments to retry.", "info"); return; }
    STATE.halt=false;
    document.getElementById('btn-halt').style.display='inline-block';
    LOG.add(`Retrying ${failed.length} failed segment${failed.length===1?'':'s'}...`, 'warning');
    const affectedChapters = new Set();
    await runPool(failed, getGenConcurrency(), async ({i,j}) => {
        if(STATE.halt) return;
        const ch = STATE.chapters[i]; if(!ch) return;
        const ok = await processChunkGeneration(ch.chunks[j], i, j);
        if(ok) affectedChapters.add(i);
    });
    const silenceChunk = parseFloat(dom.silenceChunk.value) || 0;
    for(const i of affectedChapters){
        const ch = STATE.chapters[i];
        const files = (ch.chunks||[]).map(getChunkFileName).filter(Boolean);
        if(files.length === (ch.chunks||[]).length){
            try{
                const r = await APIService.mergeChapter(STATE.project.id, i, files, ch.title==="Titles", silenceChunk);
                ch.audioUrl = r.url; queueAutoSave(`chapter-retry-${i+1}`);
            }catch(e){ LOG.add(`Re-merge failed for chapter ${i}: ${e.message}`, 'error'); }
        }
    }
    document.getElementById('btn-halt').style.display='none';
    renderTimeline();
    LOG.add("Retry pass complete.", "success");
}
...
dom.btnPlay.addEventListener('click',async()=>{if(STATE.isPlaying){engine.stop();return}
    ...
    for(let i=0;i<tot;i++){
        if(!STATE.isPlaying)break;const c=q[i];STATE.activePlaybackId=`chunk_${c.id}`;updatePlaybackHighlight();dom.masterProgress.value=((i+1)/tot)*100;
...
function createProjectFromSettings() {
    ...
    STATE.project.id = generateProjectId(STATE.projectMeta);
    STATE.project.sourceHash = '';
    STATE.project.analyzedOnce = false;
    resetAutoSaveTracker(false);
    ...
    STATE.project.notes = "";
    const headerNameReset = document.getElementById('header-project-name');
    if(headerNameReset) headerNameReset.value = "";
...
async function loadProject(id){
    ...
    try{
        ...
        // V13: keep the saved project's unique id authoritative so re-generation hits its cache.
        STATE.project.id = d.id || (d.projectSettings && d.projectSettings.id) || STATE.project.id;
        STATE.project.customTitle = d.displayTitle || d.title || '';
        if(STATE.project.analyzedOnce === undefined) STATE.project.analyzedOnce = true;
        STATE.projectMeta={
        ...
        if(STATE.project.modelId){ if(STATE.project.modelId==='eleven_multilingual_v3') STATE.project.modelId='eleven_v3'; dom.elModel.value = STATE.project.modelId; }
        ...
        // N5: restore heading-narration toggle. N8: restore session spend.
        const speakHeadingsLoad = document.getElementById('speak-headings');
        if (speakHeadingsLoad) speakHeadingsLoad.checked = STATE.project.speakHeadings !== false;
        STATE.sessionCost = Number(d.sessionCost) || 0;
        // V13.1: restore parallel-requests setting
        const gcLoad = document.getElementById('gen-concurrency');
        if (gcLoad && STATE.project.genConcurrency) gcLoad.value = STATE.project.genConcurrency;
        const projectTitleInput = document.getElementById('project-title-input');
        ...
        const loadCounts = summarizeChapterCounts();
        document.getElementById('sum-chapters').innerText = `${loadCounts.chapters}${loadCounts.extras.length ? ' +' + loadCounts.extras.join(',') : ''}`;
        document.getElementById('sum-chars').innerText = totalChars.toLocaleString();
...
// ============================================================
// V13: Document import (.txt / .md / .docx) + drag-and-drop
// ============================================================
async function importTextFromFile(file){
    if(!file) return;
    const name=(file.name||'').toLowerCase();
    try{
        let text='';
        if(name.endsWith('.docx')){
            if(typeof mammoth==='undefined'){ LOG.add('DOCX support is unavailable (mammoth.js failed to load).','error'); return; }
            LOG.add(`Reading ${file.name} ...`);
            const buf=await file.arrayBuffer();
            const result=await mammoth.extractRawText({arrayBuffer:buf});
            text=result.value||'';
            (result.messages||[]).slice(0,3).forEach(m=>LOG.add(`DOCX note: ${m.message}`,'info'));
        } else if(name.endsWith('.txt')||name.endsWith('.md')||name.endsWith('.markdown')||(file.type&&file.type.indexOf('text')===0)){
            text=await file.text();
        } else {
            LOG.add(`Unsupported file type: ${file.name}. Please use .txt, .md or .docx.`,'warning');
            return;
        }
        text=(text||'').replace(/\r\n/g,'\n').trim();
        if(!text){ LOG.add(`No readable text found in ${file.name}.`,'warning'); return; }
        dom.manuscript.value=text;
        localStorage.setItem('ab_manuscript', text);
        updateReceipt();
        LOG.add(`Imported "${file.name}" (${text.length.toLocaleString()} chars). Now click "1. Analyze & Parse".`,'success');
    }catch(e){
        LOG.add(`Import failed: ${e.message||e}`,'error');
    }
}

// Wire up all the new V13 controls once the DOM is present.
(function setupV13Wiring(){
    // Inline project-name field in the header <-> STATE.project.customTitle
    const headerName=document.getElementById('header-project-name');
    if(headerName){
        headerName.addEventListener('input', ()=>{ STATE.project.customTitle = headerName.value.trim(); });
        headerName.addEventListener('change', ()=>{ STATE.project.customTitle = headerName.value.trim(); if(STATE.chapters.length) queueAutoSave('rename'); });
        syncHeaderProjectName();
    }
    ...
    if(importBtn&&fileInput){
        ...
        fileInput.addEventListener('change', e=>{ const f=e.target.files&&e.target.files[0]; importTextFromFile(f); fileInput.value=''; });
    }
    // Drag-and-drop onto the manuscript editor
    const editor=document.querySelector('.editor-container');
    if(editor){
        ['dragenter','dragover'].forEach(ev=>editor.addEventListener(ev, e=>{ e.preventDefault(); e.stopPropagation(); editor.classList.add('drag-over'); }));
        ['dragleave','dragend'].forEach(ev=>editor.addEventListener(ev, e=>{ e.preventDefault(); e.stopPropagation(); editor.classList.remove('drag-over'); }));
        editor.addEventListener('drop', e=>{ e.preventDefault(); e.stopPropagation(); editor.classList.remove('drag-over'); const f=e.dataTransfer&&e.dataTransfer.files&&e.dataTransfer.files[0]; if(f) importTextFromFile(f); });
    }
    // Retry Failed button
    const retryBtn=document.getElementById('btn-retry-failed');
    if(retryBtn) retryBtn.addEventListener('click', retryFailedChunks);
})();
