import { startJourney, event, setNetwork, setWallet, classify } from './telemetry';
...
// The wallet bridge speaks the v6 wire helpers that @stacks/connect 7.x is
// built around; the collections client speaks @stacks/transactions v7. Both
// are installed (v6 under the `@stacks/transactions-v6` alias) and the seam
// between them is `services/submit.ts`, which hands this layer pre-serialized
// hex strings so no Clarity value ever crosses the version boundary.
import {
  AnchorMode,
  createAddress,
...
const normalizeNetwork = (value: unknown, fallback: NetworkType = getActiveNetwork()): NetworkType => {
  if (typeof value === 'string') {
...
const deriveWalletSession = (userData: UserData): WalletSession => {
  ...
  // CHANGED vs. the v15.1 original: the app is no longer mainnet-only. A
  // session is accepted when its address is encoded for the network the app is
  // currently configured for (mainnet in production, testnet for the canary).
...
const toWalletSession = (
  payload: unknown,
  fallbackNetwork: NetworkType = getActiveNetwork()
): WalletSession => {
  const address = extractStacksAddress(payload);
  if (!address) {
    return disconnectedSession();
  }
  // CHANGED vs. the v15.1 original: configurable network, see deriveWalletSession.
