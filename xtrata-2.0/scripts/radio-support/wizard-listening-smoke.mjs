...
try{for(const width of [1200,390]){
 ...
 try{
  ...
  await page.getByText('Paid mainnet test settings',{exact:true}).click();await page.locator('#radio-paid-max').fill('2');await page.locator('#radio-approve').check();await page.locator('#radio-enable').click();
  await page.waitForFunction(()=>document.getElementById('radio-mode').textContent.startsWith('PAID TEST'));assert.equal(sent,0);
  ...
  await page.locator('#listening-section').screenshot({path:join(output,`player-${width}.png`)});
