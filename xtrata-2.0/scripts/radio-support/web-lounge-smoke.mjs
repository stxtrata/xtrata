...
try{
 ...
 await browser.route('https://xtrata.xyz/**',async route=>{const path=new URL(route.request().url()).pathname;if(path==='/radio/counts')return route.fulfill({json:{tracks:[]}});if(path==='/xtrata-radio.js')return route.fulfill({contentType:'text/javascript',body:'window.XtrataRadio={subscribe:f=>f({playing:false,volumeStep:8}),playPause(){},next(){},prev(){}};'});const file=path==='/radio/lounge'?'/radio/lounge.html':path;try{const body=(await readFile('public'+file,'utf8')).replaceAll('8798',String(port));return route.fulfill({body,contentType:file.endsWith('.html')?'text/html':file.endsWith('.js')?'text/javascript':'text/css'});}catch{return route.abort();}});
 const page=await browser.newPage();await page.goto('https://xtrata.xyz/radio/lounge');
 ...
 const startId='d'.repeat(32);await page.evaluate(id=>window.dispatchEvent(new CustomEvent('xtrata:radio-audible-start',{detail:{song:315,id}})),startId);await page.waitForTimeout(300);await rpc('start',{song:315,startId});assert.equal(paid,1);await page.locator('#free').click();await page.getByText('FREE PLAY · support off',{exact:true}).waitFor();assert.equal((await rpc('status')).enabled,false);
