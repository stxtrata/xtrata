...
export class MusicWebBridge {
 ...
 const {method}=data;const keys={pair:['method'],poll:['method','id'],status:['method','token'],setup:['method','token'],support:['method','token','fee'],start:['method','token','id','song','duration'],stop:['method','token'],disconnect:['method','token']};
 if(!keys[method]||Object.keys(data).sort().join()!==keys[method].sort().join())throw Error('Unsupported bridge request.');
 ...
 if(!validId(data.id)||!Number.isSafeInteger(data.song)||data.song<0||!Number.isFinite(data.duration)||data.duration<60)throw Error('Invalid start.');
 ...
 try{await this.media.audio(data.song);this.connection(origin,data.token);if(c.approval!==approval)throw Error('Support stopped.');const result=await this.listening.start({...approval,id:data.id,song:data.song,duration:data.duration});c.starts.set(data.id,result);return result;}finally{this.preparing=false;}
 }
}
