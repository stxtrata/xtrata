...
if(!['setup','status','prepare','register','replace','launch','retire'].includes(command))throw new Error('Use setup, status, prepare, register, replace or launch. This wrapper never invokes inscription mode.');
const security=(args)=>spawnSync('/usr/bin/security',args,{encoding:'utf8'});
let secret;
if(['prepare','register','replace','launch','retire'].includes(command)){
...
const result=spawnSync(process.execPath,[join(root,'collection-v15-live.mjs'),command,...(['prepare','register','replace','launch','retire'].includes(command)?['--broadcast']:[])],{env:{...process.env,...(apiEnv.HIRO_API_KEY?{HIRO_API_KEY:apiEnv.HIRO_API_KEY}:{}),...(secret?{COLLECTION_WIZARD_PASSPHRASE:secret}:{})},encoding:'utf8',stdio:'inherit'});
