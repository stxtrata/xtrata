...
it('keeps current downloads and renders beta versions in a closed disclosure',async()=>{
 ...
 vi.stubGlobal('fetch',vi.fn(async()=>({ok:true,json:async()=>({channel:'preview',downloads:[entry('1.0.6')],betaDownloads:[entry('1.0.7')]})})));
 new Function(script)();
 await vi.waitFor(()=>expect(document.querySelector('#beta-downloads-list')!.textContent).toContain('1.0.7'));
