...
(function(){
  ...
  var adminBusy = false;
  var WALLET_DEBUG_BUILD = 'wallet-v3-sandbox-2026-07-16-1';
  ...
  function maybePromptWalletConnectForOnChain(){
    ...
    /* v3: window.confirm is silently ignored in sandboxed iframes, so the
       prompt is a non-blocking notice instead. The player connects via the
       always-visible Connect button; guest play (local PB) is the default. */
    setTimeout(function(){
      arcadeNotify(
        'Playing as guest (local personal best only). Use the Connect button for on-chain Top 10 eligibility.',
        { durationMs: 8000 }
      );
      walletDebug('info', 'Wallet connect notice shown (non-blocking, v3)');
    }, 0);
  }
  ...
  async function connectWalletInternal(){
    ...
    /* v3: if the primary provider is the host wallet bridge shim (installed by
       the recursive parent inside sandboxed embeds), NEVER run @stacks/connect
       here - popups are blocked in the sandbox and showConnect would hang.
       The shim's own connect methods route to the host page, which runs the
       real wallet prompt in a context where it can succeed. */
    if(
      primaryAuthEntry &&
    ...
    /* v3: Xverse's legacy XverseProviders.StacksProvider.request is an
       unimplemented stub; its real RPC surface is BitcoinProvider.request and
       the ONLY reliable connect method is a single wallet_connect (see
       CHANGELOG-2.0 "Free sponsored claims - Xverse/Leather signing
       correction", 2026-07-14). Promote BitcoinProvider-style candidates and
       lead with wallet_connect for them instead of probing nine methods. */
    connectProviders = connectProviders.slice().sort(function(a, b){
      var ax = /bitcoinprovider/i.test(a.label || '') ? 0 : 1;
      var bx = /bitcoinprovider/i.test(b.label || '') ? 0 : 1;
      return ax - bx;
    });
  ...
  /* v3: sandbox-safe notice toast. window.alert/confirm are silently ignored
     inside sandboxed iframes (no allow-modals), so all user messaging must be
     in-DOM. Notices stack, auto-dismiss, and are dismissible by click. */
  var noticeStackEl = null;
  function arcadeNotify(message, opts){
    try{
      var options = opts || {};
      if(!noticeStackEl){
        noticeStackEl = document.createElement('div');
        noticeStackEl.id = 'arcade-notice-stack';
        noticeStackEl.setAttribute('role', 'status');
        noticeStackEl.setAttribute('aria-live', 'polite');
        noticeStackEl.style.cssText =
          'position:fixed;top:52px;right:12px;z-index:99999;display:flex;' +
          'flex-direction:column;gap:8px;max-width:min(92vw,360px);';
        (document.body || document.documentElement).appendChild(noticeStackEl);
      }
      var item = document.createElement('div');
      item.style.cssText =
        'background:#0e142b;border:1px solid ' +
        (options.tone === 'error' ? '#7a2d3a' : '#33406f') +
        ';color:' + (options.tone === 'error' ? '#ffb4b4' : '#d6e7ff') +
        ';border-radius:10px;padding:10px 12px;font-size:13px;line-height:1.45;' +
        'box-shadow:0 8px 24px rgba(0,0,0,0.45);cursor:pointer;word-break:break-word;' +
        'font-family:inherit;';
      item.textContent = String(message == null ? '' : message);
      if(options.detail){
        var detail = document.createElement('div');
        detail.style.cssText = 'margin-top:6px;font-size:11px;color:#9ec6ff;word-break:break-all;';
      ...
      noticeStackEl.appendChild(item);
      setTimeout(removeItem, Math.max(3500, Number(options.durationMs || 9000)));
    }catch(error){
      walletDebug('warn', 'arcadeNotify failed', walletErrorForLog(error));
    }
  }

  /* v3: when no wallet provider exists in this context (sandboxed embed with
     no host bridge), route the player to the secure runtime page instead of
     failing. The host opens it via the open-runtime intent; if no host
     listens, the URL is shown so the player can open it manually. */
  function isNoProviderError(error){
    var msg = error && error.message ? String(error.message) : String(error || '');
    return msg.indexOf('No Stacks-compatible wallet provider') >= 0;
  }
  function handleWalletUnavailable(context){
    var bridge = typeof window !== 'undefined' ? window.ArcadeHostBridge : null;
    if(bridge && typeof bridge.requestRuntimeOpen === 'function'){
      var runtimeUrl = '';
      try{
        runtimeUrl = bridge.requestRuntimeOpen({ reason: context || 'wallet-connect' });
      }catch(error){}
      arcadeNotify(
        'No wallet is reachable in this embedded view. Asking the Xtrata host to open the secure runtime - approve the new window, then connect your wallet there.',
        { detail: runtimeUrl, durationMs: 14000 }
      );
      return true;
    }
    arcadeNotify(
      'No Stacks wallet detected here. Open the game on xtrata.xyz (secure runtime) to connect a wallet and submit scores.',
      { tone: 'error', durationMs: 14000 }
    );
    return false;
  }
    ...
    walletStatusBadge.onclick = function(){
      ...
      connectWallet().catch(function(error){
        ...
        if(isNoProviderError(error)){
          handleWalletUnavailable('wallet-badge-connect');
        }else{
          arcadeNotify('Wallet connect failed: ' + msg, { tone: 'error' });
        }
      });
    };
  }
  ...
  if(walletDisconnectBtn){
    walletDisconnectBtn.onclick = function(){
      ...
      disconnectWallet().catch(function(error){
        ...
        arcadeNotify('Wallet disconnect failed: ' + msg, { tone: 'error' });
      });
    };
    updateWalletDisconnectButtonState();
  }
  ...
  if(walletConnectBtn){
    walletConnectBtn.onclick = function(){
      ...
      connectWallet().catch(function(error){
        ...
        if(isNoProviderError(error)){
          handleWalletUnavailable('wallet-connect-btn');
        }else{
          arcadeNotify('Wallet connect failed: ' + msg, { tone: 'error' });
        }
      });
