 ...
 * Generated At: 2026-02-20T20:52:02.985Z
 * Legacy Source SHA256: 9ce6f9b4600c3de8ea0393cea36e964aafb57f937284dbe8d256f2b7a6c9c8b8
 */
...
var Game01 = (function(){
  ...
  var controls = 'Arrows: Move (Up/Down unlock later), Space: Shoot, P: Pause, R: Restart';
  var hasLevels = true;
  var scoreMode = 'score';
  ...
  function ensurePauseButton(){
    ...
    if(pauseButton && pauseButton.parentNode === container){
      return pauseButton;
    }
    ...
    pauseButton.className = 'ab-pause-btn';
    pauseButton.textContent = 'Pause';
    pauseButton.style.position = 'absolute';
    pauseButton.style.right = '12px';
    pauseButton.style.top = '12px';
    pauseButton.style.padding = '8px 14px';
    pauseButton.style.border = '1px solid #6fd5ff';
    pauseButton.style.borderRadius = '8px';
    pauseButton.style.background = 'rgba(3, 22, 38, 0.9)';
    pauseButton.style.color = '#d6f6ff';
    pauseButton.style.font = '12px monospace';
    pauseButton.style.letterSpacing = '0.3px';
    pauseButton.style.cursor = 'pointer';
    pauseButton.style.boxShadow = '0 0 12px rgba(57, 191, 255, 0.22)';
    pauseButton.style.zIndex = '22';
  ...
  function refreshPauseButton(){
    var btn = ensurePauseButton();
    if(!btn) return;
    var paused = !!(state && state.paused);
    btn.textContent = paused ? 'Resume' : 'Pause';
    btn.title = paused ? 'Resume gameplay' : 'Pause gameplay';
    btn.style.opacity = state && state.gameOver ? '0.6' : '1';
  }
  ...
  function drawHud(){
    ...
    if(state.mobilityUnlockIntro > 0){
      var unlockAlpha = Math.min(0.88, state.mobilityUnlockIntro / 180);
      ctx.globalAlpha = unlockAlpha;
      ctx.fillStyle = 'rgba(2,6,16,0.82)';
      ctx.fillRect(0, HEIGHT * 0.5 - 26, WIDTH, 52);
      ctx.fillStyle = '#89f4ff';
      ctx.font = '18px monospace';
      ctx.textAlign = 'center';
      ctx.fillText('VERTICAL THRUSTERS ONLINE', WIDTH * 0.5, HEIGHT * 0.5 + 7);
      ctx.textAlign = 'left';
      ctx.globalAlpha = 1;
    }
  }
  ...
  function drawPauseOverlay(){
    ...
    ctx.fillText('Press P or use the Pause button to resume', WIDTH * 0.5, HEIGHT * 0.42 + 58);
    ctx.textAlign = 'left';
  }
...
(function(game){
  ...
  (function(){
    function createRewardsHazardsRuntime(config){
      ...
      function markDrop(cadenceState, context, pickupType, spreadType, thrusterType){
      ...
      function resolveDrop(payload){
        ...
        if(cadenceState && !starvationActive){
          if(killsSinceDrop < getMinKillsBetweenDrops(context.level)) return null;
        ...
        if(!starvationActive && hazardChance > 0 && rand() < hazardChance && hazards.length > 0){
          ...
          markDrop(cadenceState, context, hazardDrop.pickupType, spreadType, thrusterType);
          return hazardDrop;
        }
        ...
        if(
          ...
          markDrop(cadenceState, context, thrusterDrop.pickupType, spreadType, thrusterType);
          return thrusterDrop;
        }
        ...
        if(chooseSpread){
          ...
          markDrop(cadenceState, context, spreadDrop.pickupType, spreadType, thrusterType);
          return spreadDrop;
        }
        ...
        markDrop(cadenceState, context, lifeDrop.pickupType, spreadType, thrusterType);
        return lifeDrop;
      }
    ...
    function patchGameInitDestroy(){
      ...
      game.init = function(container, shared){
        ...
        if(canUseDom() && container){
          game.__astroThreatBriefingOverlay = installThreatBriefingOverlay(game, container);
        }
