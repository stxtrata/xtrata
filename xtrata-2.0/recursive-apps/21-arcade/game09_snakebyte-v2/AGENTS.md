# AGENTS - game09_snakebyte-v2 Workspace

This file governs updates inside `recursive-apps/21-arcade/game09_snakebyte-v2/`.

## Core Rules
1. Preserve launcher contract invariants:
   - Global symbol remains `Game09`.
   - `id` remains `snakebyte`.
   - `scoreMode` must stay aligned with live slot behavior unless explicitly changed.
2. Treat `recursive-apps/21-arcade/games/game09_snakebyte-v2.js` as generated output only.
...
2. Before promoting a version, run strict review:
   - `npm run arcade:strategy:review -- --game game09_snakebyte --strict`
...
2. Run this workspace build command (add in root `package.json` if missing):
   - `npm run arcade:snakebyte:v2:build`
3. Run this workspace test command (add in root `package.json` if missing):
   - `npm run arcade:snakebyte:v2:test`
4. Validate generated JS:
   - `node --check recursive-apps/21-arcade/games/game09_snakebyte-v2.js`

## Promotion to Runtime `games/`
1. Confirm workspace build and tests pass.
2. Ensure output exists at `recursive-apps/21-arcade/games/game09_snakebyte-v2.js`.
3. Regenerate manifest:
   - `npm run arcade:games:manifest`
4. Launch arcade and verify slot `09` resolves to latest file.
...
2. Get suggested next target:
   - `npm run arcade:next-version -- --game game09_snakebyte`
3. Explicit major jump only when requested:
   - `npm run arcade:next-version -- --game game09_snakebyte --version 3`
