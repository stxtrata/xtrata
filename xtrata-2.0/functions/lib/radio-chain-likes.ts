...
async function read(contract:string,fn:string,args:Parameters<typeof serializeCV>[0][],sender:string){
 const [address,name]=contract.split('.');
 const response=await fetch(`${API}/v2/contracts/call-read/${address}/${name}/${fn}`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({sender,arguments:args.map(a=>'0x'+Array.from(serializeCV(a)).map(b=>b.toString(16).padStart(2,'0')).join(''))}),signal:AbortSignal.timeout(12000)});
...
export async function chainConfig(env:Env){
 ...
 const result=await read(contract,'get-config',[],contract.split('.')[0]);const v=result.value;
...
export async function chainStates(contract:string,ids:number[],listener=contract.split('.')[0]){
 ...
 const result=await read(contract,'get-state',[Cl.list(ids.map(n=>Cl.uint(n))),Cl.standardPrincipal(listener)],listener);
...
export async function chainTransaction(contract:string,txid:string,listener:string){
 if(!/^0x[0-9a-f]{64}$/i.test(txid)||!validateStacksAddress(listener))throw Error('Invalid transaction');
 const response=await fetch(`${API}/extended/v1/tx/${txid}?unanchored=false`,{signal:AbortSignal.timeout(12000)});
