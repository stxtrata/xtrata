...
export async function refreshRadioMetadata(env:Env) {
 try {
  ...
  const rows=await queryAll(env,`SELECT i.token_id FROM inscription_index i LEFT JOIN radio_metadata m ON m.token_id=i.token_id
   WHERE i.contract=? AND i.sealed=1 AND i.mime LIKE 'text/html%' AND (m.token_id IS NULL OR (m.status='ready' AND m.cover_checked=0) OR (m.status!='ready' AND m.checked_at<?))
  ...
  for(const row of (rows.results||[]) as {token_id:number}[]) {
   ...
   try {
    ...
    await run(env,"UPDATE radio_metadata SET title=?,artist=?,cover=?,cover_checked=1,status='ready' WHERE token_id=?",[meta.title,meta.artist,inscriptionArtwork(html),row.token_id]);
