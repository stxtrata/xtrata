...
export async function refreshRadioMetadata(env:Env) {
 try {
  ...
  const rows=await queryAll(env,`SELECT i.token_id FROM inscription_index i LEFT JOIN radio_metadata m ON m.token_id=i.token_id
   WHERE i.contract=? AND i.sealed=1 AND i.mime LIKE 'text/html%' AND (m.token_id IS NULL OR (m.status='ready' AND (m.cover_checked=0 OR m.is_song IS NULL OR m.album IS NULL)) OR (m.status!='ready' AND m.checked_at<?))
  ...
  for(const row of (rows.results||[]) as {token_id:number}[]) {
   ...
   const claim=await queryAll(env,`INSERT INTO radio_metadata(token_id,checked_at,status) VALUES(?,?,'pending')
    ...
    WHERE (radio_metadata.status='ready' AND (radio_metadata.cover_checked=0 OR radio_metadata.is_song IS NULL OR radio_metadata.album IS NULL)) OR (radio_metadata.status!='ready' AND radio_metadata.checked_at<?) RETURNING token_id`,[row.token_id,now,now-86400000]);
   ...
   try {
    ...
    await run(env,"UPDATE radio_metadata SET title=?,artist=?,album=?,cover=?,cover_checked=1,is_song=?,status='ready' WHERE token_id=?",[meta.title,meta.artist,meta.album,inscriptionArtwork(html),Number(inscriptionHasAudio(html)),row.token_id]);
