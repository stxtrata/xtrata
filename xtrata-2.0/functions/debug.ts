...
(function(){
  ...
  function renderKPIs(k){var g=h("div",{class:"tiles"});g.appendChild(tile("Crash-outs",n(k.crashOuts),"Times someone hit an error they could not get past."));g.appendChild(tile("Unique issues",n(k.distinctIssues),"Distinct problems behind those crash-outs."));g.appendChild(tile("People affected",n(k.sessionsAffected),"Separate visits that hit at least one error."));g.appendChild(tile("Recovery rate",pct(k.recoveryPct),"Of visits that errored, how many eventually succeeded."));return g;}

  function renderByFlow(list){if(!list||!list.length)return h("p",{class:"muted",text:"No activity recorded in this period yet."});var max=0;for(var i=0;i<list.length;i++)max=Math.max(max,Number(list[i].errors)||0);if(max===0)max=1;var box=h("div",{class:"card"});for(var j=0;j<list.length;j++){var f=list[j];var w=Math.round(((Number(f.errors)||0)/max)*100);var track=h("div",{class:"track"},h("div",{class:"fill",style:"width:"+w+"%"}));box.appendChild(h("div",{class:"barrow",title:n(f.events)+" total events in "+flowName(f.flow)},h("div",{class:"name",text:flowName(f.flow)}),track,h("div",{class:"val",text:n(f.errors)})));}return box;}
  ...
  function paint(d){
    ...
    if((d.kpis&&d.kpis.totalEvents===0)){bannerEl.appendChild(banner("info","No events recorded in this period yet. Once the site is live with logging, crash-outs and journeys will appear here."));}
    content.appendChild(renderKPIs(d.kpis||{}));
    content.appendChild(section("Where it is going wrong","Number of crash-outs by area of the site. Longer bar = more failures happening there.",renderByFlow(d.byFlow)));
