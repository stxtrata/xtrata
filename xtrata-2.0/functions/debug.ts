...
(function(){
  ...
  var CODES={
    ...
    WALLET_RPC_INTERNAL:{label:"Wallet could not construct the transaction",plain:"The wallet returned a generic internal error while preparing the transaction. Inspect the provider code, function, and payload details before retrying."},
  ...
  function issueDetail(it){var info=codeInfo(it.errorCode);var box=h("div",{class:"box"});box.appendChild(h("p",{class:"plain",text:info.plain}));if(it.sample)box.appendChild(h("div",{class:"sample",text:String(it.sample)}));box.appendChild(h("div",{class:"kv"},h("span",{},[h("b",{text:n(it.occurrences)})," times"]),h("span",{},[h("b",{text:n(it.sessions)})," people"]),h("span",{},[h("b",{text:n(it.wallets)})," wallets"]),h("span",{},["last seen ",h("b",{text:timeAgo(it.lastSeen)})]),h("span",{},["where: ",h("b",{text:flowName(it.flow)+" · "+stepName(it.step)})])));var tech=[];if(it.appVersion)tech.push("release: "+it.appVersion);if(it.walletKind)tech.push("wallet: "+it.walletKind);if(it.browser||it.os||it.device)tech.push("client: "+[it.browser,it.os,it.device].filter(Boolean).join(" / "));if(it.route)tech.push("route: "+it.route);if(it.context)tech.push("context: "+it.context);if(it.stack)tech.push("stack: "+it.stack);if(tech.length){var details=h("details");details.appendChild(h("summary",{text:"Technical details (latest occurrence)"}));details.appendChild(h("div",{class:"sample",text:tech.join("\n")}));box.appendChild(details);}return box;}
