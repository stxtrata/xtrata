...
- Initial address discovery uses Connect's parameter-free `connect()` path. Connect maps Xverse discovery to `wallet_connect`, where the network hint is optional; mainnet is enforced consistently from the returned STX address instead.
