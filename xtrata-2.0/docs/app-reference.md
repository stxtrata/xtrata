...
- `functions/collection-drop/registry.ts` reconstructs the public, claimed-only campaign edition registry from Drops v1.1 print events. It converts the contract's zero-based campaign editions to the recursive master's 1–1024 edition numbers, returns CORS JSON, and edge-caches the event scan for the claim-gated Proof of Free mosaic.
