...
- `src/lib/viewer/public-wallet-bridge.ts` owns wallet connection and native STX payment requests from explicitly registered interactive homepage previews. `src/home/main.js` registers selected-inscription and prepared-file previews, including their fullscreen frames; grid thumbnails are excluded. Preserve frame-bound tokens, host review, account/network checks, the shared busy state and the sandbox. See `docs/timeloop-public-wallet-bridge.md` for validation and release limits.
