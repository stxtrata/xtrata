...
**Why this is first, and why it is not a milestone.** Reading art out of an arbitrary collection is the hardest engineering in the grant and everything downstream waits on it. There is no standard for where NFT art lives. Bitcoin Pepes gives a complete reference set to test against, for free, because it is fully preserved and every one of its 2,089 hashes was validated by the contract at mint time rather than merely recorded. LEO Cats offers 101 such validations and Miami Degens one, so Pepes is the only collection that can check the harvester across a whole collection rather than a handful of tokens. If this takes four weeks instead of two I want to know that in September with eight weeks left, not in October with two. It is a checkpoint rather than a milestone because it is infrastructure, not something a member of the public can use yet.
