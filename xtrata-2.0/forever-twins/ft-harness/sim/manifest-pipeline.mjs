...
import { buildAll, manifestDoc, manifestText, chunk, MAX_BYTES, xtrataHashHex } from '../manifest/lib.mjs';
...
const sizes = [900, 16384, 16385, 40000, 70001, 120000];
ids.forEach((id, k) => {
...
scenario('M-2', 'refusals: over 512 KB, missing art, partial manifest');
web.set(`https://gw.test/ipfs/QmMeta/999.json`, new TextEncoder().encode(JSON.stringify({ image: 'ipfs://QmArt/999.png' })));
web.set(`https://gw.test/ipfs/QmArt/999.png`, png(MAX_BYTES + 1, 9));
const big = await buildAll(cfg, [999, 998], fetcher);
check('token over 512 KB flagged as oversize', big.oversize.length === 1 && big.oversize[0].id === 999, JSON.stringify(big.oversize));
...
try { const d = JSON.parse(text); d.tokens[0].twin.totalSize = MAX_BYTES + 1; makePlan(manifestText(d), H); refused = ''; } catch (e) { refused = e.message; }
