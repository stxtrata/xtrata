;; forever-twin-helper-v3 :: ordinal-pepe :: group G2
;; ---------------------------------------------------------------------------
...
(define-constant COLLECTION-KEY "ordinal-pepe")
(define-constant MASTER .xtrata-v3-2-3)
(define-constant SOURCE .ordinal-pepe)
...
(define-private (source-owner (token-id uint))
  (unwrap-panic (contract-call? .ordinal-pepe get-owner token-id)))
...
(define-private (source-listed (token-id uint))
  (is-some (contract-call? .ordinal-pepe get-listing-in-ustx token-id)))
...
(define-private (release-original-to (id uint) (recipient principal))
  (as-contract? ((with-nft SOURCE "ordinal-pepe" (list id)))
...
(define-read-only (get-twin-interface)
  { interface-version: INTERFACE-VERSION, collection-key: COLLECTION-KEY, master: MASTER, source: SOURCE,
    source-asset: "ordinal-pepe", route: "standard", group: "G2",
