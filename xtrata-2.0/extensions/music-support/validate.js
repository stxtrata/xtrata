export function validate(message,sender,extension){
 ...
 const fields={connect:[['id','method']],poll:[['id','method']],status:[['id','method']],setup:[['id','method']],support:[['id','method','fee']],start:[['id','method','song','startId'],['duration','id','method','song','startId']],stop:[['id','method']],disconnect:[['id','method']]};
 if(!message||!fields[message.method]||!fields[message.method].some(allowed=>Object.keys(message).sort().join()===allowed.sort().join())||!/^[a-f0-9]{32}$/.test(message.id||''))throw Error('Invalid request');
