export function validate(message,sender,extension){
 ...
 const fields={connect:['id','method'],poll:['id','method'],status:['id','method'],setup:['id','method'],support:['id','method','fee'],start:['duration','id','method','song','startId'],stop:['id','method'],disconnect:['id','method']};
 ...
 if(message.method==='start'&&(!Number.isSafeInteger(message.song)||message.song<0||!/^[a-f0-9]{32}$/.test(message.startId||'')||!Number.isFinite(message.duration)||message.duration<60))throw Error('Invalid start');
 return message;
}
