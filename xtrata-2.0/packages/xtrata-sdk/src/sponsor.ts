/**
 * Client for the Xtrata sponsor relayer.
 *
 * Lets a buyer purchase a sponsored listing with zero STX: the buyer's wallet
 * signs the market `buy` as a sponsored transaction (fee 0), the client POSTs
 * the signed payload to the relayer, and the relayer attaches + pays the
 * mining fee, then settles the seller's fee-budget escrow (claim + dust
 * refund) on-chain.
 *
 * Framework independent: inject any `fetch` implementation. The first-party
 * app consumes this module; third-party integrators can point `baseUrl` at
 * any deployment of the sponsor service.
 *
 * Invariant (wallet UX): the sponsored transaction MUST be signed over the
 * same wallet provider bridge as the active session. Mixing providers (e.g.
 * session via one Xverse bridge, signing via another) surfaces as a
 * "Network mismatch" wallet error.
 */
