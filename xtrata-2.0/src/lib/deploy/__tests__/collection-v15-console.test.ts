...
import source from '../../../../contracts/live/xtrata-collection-mint-v1.5.clar?raw';
import {COLLECTION_V15_NAME, COLLECTION_V15_CORE} from '../collection-v15-canary';
...
const card=()=>document.querySelector('#collection-v15-deployment') as HTMLElement;
...
beforeEach(async()=>{
 ...
 vi.stubGlobal('fetch',vi.fn(async(url:any)=>{
  ...
  if(path.includes('/interface/')&&path.endsWith('/'+COLLECTION_V15_NAME))return new Response('{}',{status:state==='unavailable'?503:state==='missing'?404:200});
  ...
  if(path.includes('/call-read/')){
   ...
   const value=fn==='get-locked-core-contract'?Cl.contractPrincipal(deployer,'xtrata-v3-2-3'):fn==='is-paused'?Cl.bool(corePaused):fn==='get-max-small-mint-chunks'?Cl.uint(30):Cl.uint(0);
...
describe('collection v1.5 wallet deployment',()=>{
 ...
 it('submits the pinned source as Clarity 4 once and handles cancellation',async()=>{
  ...
  expect(args).toMatchObject({contractName:COLLECTION_V15_NAME,codeBody:source,clarityVersion:4,fee:490000n,stxAddress:deployer});
 ...
 it('verifies an existing deployment and logs state without sending admin calls',async()=>{state='live';await preflight();expect(card().textContent).toContain('Deployed source and state verified');expect(card().textContent).toContain(COLLECTION_V15_CORE);expect(wallet.call).not.toHaveBeenCalled();});
});
