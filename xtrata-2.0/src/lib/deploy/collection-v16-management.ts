...
export function renderCollectionManagement(run: (name: string, args: ClarityValue[], write: boolean) => Promise<unknown>) {
  ...
  const root = details('Collection v1.6 — manage contract & testing');
  const stages = [
    ['1. Local regression tests','Run permissions, split limits, phase dates/caps, reservation expiry, duplicate rejection and one-time supply/finalization cases locally. Command: npm --prefix contracts/clarinet test -- tests/xtrata-collection-mint-v1.6.test.ts'],
