...
export type ParsedVaultContract = {
  config: ContractConfig | null;
  error: string | null;
};

export const parseVaultContractId = (value: string): ParsedVaultContract => {
  const trimmed = value.trim();
  if (!trimmed) {
    return { config: null, error: 'Set a vault contract ID first.' };
  }
