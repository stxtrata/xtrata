...
export type ParsedPreinscribedSaleContract = {
...
export const parsePreinscribedSaleContractId = (
  value: string
): ParsedPreinscribedSaleContract => {
  const trimmed = value.trim();
  if (!trimmed) {
    return {
      config: null,
      error: 'Set a pre-inscribed sale contract ID first.'
    };
  }
  ...
  if (dotIndex <= 0 || dotIndex === trimmed.length - 1) {
    return {
      config: null,
      error: 'Use format ADDRESS.CONTRACT-NAME.'
    };
  }
