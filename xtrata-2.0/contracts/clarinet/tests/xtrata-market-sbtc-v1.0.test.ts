...
const marketContract = `${deployer}.xtrata-market-sbtc-v1-0`;
...
function mintToken(owner: string, chunkHex: string, tokenUri = "data:text/plain,xtrata-market-sbtc") {
...
describe("xtrata-market-sbtc-v1.0", () => {
  it("lists into escrow and exposes aligned read-only state", () => {
    ...
    expect(ownerAfterList).toBeOk(Cl.some(Cl.contractPrincipal(deployer, "xtrata-market-sbtc-v1-0")));
    ...
    ).toBeOk(Cl.contractPrincipal(deployer, "mock-sbtc"));
  ...
  it("transfers NFT to buyer and sBTC to seller", () => {
    unwrapOk(unpauseXtrata());
    const tokenId = mintToken(seller, "0b");
    unwrapOk(mintSbtc(buyer, 1_000n));
    ...
    const sellerBefore = getSbtcBalance(Cl.standardPrincipal(seller));
    const buyerBefore = getSbtcBalance(Cl.standardPrincipal(buyer));
    ...
    const sellerAfter = getSbtcBalance(Cl.standardPrincipal(seller));
    const buyerAfter = getSbtcBalance(Cl.standardPrincipal(buyer));
  ...
  it("applies fee bps to sBTC settlement", () => {
    unwrapOk(unpauseXtrata());
    ...
    const sellerBefore = getSbtcBalance(Cl.standardPrincipal(seller));
    const buyerBefore = getSbtcBalance(Cl.standardPrincipal(buyer));
    const ownerBefore = getSbtcBalance(Cl.standardPrincipal(deployer));
    ...
    const sellerAfter = getSbtcBalance(Cl.standardPrincipal(seller));
    const buyerAfter = getSbtcBalance(Cl.standardPrincipal(buyer));
    const ownerAfter = getSbtcBalance(Cl.standardPrincipal(deployer));
