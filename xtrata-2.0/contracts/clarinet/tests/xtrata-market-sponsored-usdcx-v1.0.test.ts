...
const marketName = "xtrata-market-sponsored-usdcx-v1-0";
const marketContract = `${deployer}.${marketName}`;
const usdcxContract = `${deployer}.mock-usdcx`;
...
function listToken(owner: string, tokenId: bigint, feeBudget: bigint = budget) {
  const result = simnet.callPublicFn(
    marketName,
    "list-token",
    [Cl.contractPrincipal(deployer, "xtrata-v2-1-0"), Cl.uint(tokenId), Cl.uint(price), Cl.uint(feeBudget)],
    owner
  ).result;
  return result;
}
...
function setup() {
  ...
  mintUsdcx(buyer, 10_000n);
  return tokenId;
}

describe("xtrata-market-sponsored-usdcx-v1.0", () => {
  ...
  it("buy transfers NFT and sBTC, marks sold, keeps budget escrowed", () => {
    ...
    const sellerUsdcx = unwrapUInt(
      unwrapOk(
        simnet.callReadOnlyFn(usdcxContract, "get-balance", [Cl.standardPrincipal(seller)], deployer).result
      )
    );
    const ownerUsdcx = unwrapUInt(
      unwrapOk(
        simnet.callReadOnlyFn(usdcxContract, "get-balance", [Cl.standardPrincipal(deployer)], deployer).result
      )
    );
    const fee = (price * 200n) / 10_000n;
    expect(sellerUsdcx).toBe(price - fee);
    expect(ownerUsdcx).toBe(fee);
  ...
  it("cancel returns NFT and full budget; cancel after sale rejected; only seller may cancel", () => {
    ...
    expectErr(
      simnet.callPublicFn(
        marketName,
        "cancel",
        [Cl.contractPrincipal(deployer, "xtrata-v2-1-0"), Cl.uint(listingId2)],
        seller
      ).result,
      ERR_ALREADY_SOLD
    );
