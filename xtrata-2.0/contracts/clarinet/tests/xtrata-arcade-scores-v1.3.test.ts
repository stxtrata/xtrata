...
const contract = `${deployer}.xtrata-arcade-scores-v1-3`;
...
describe('xtrata-arcade-scores-v1.3', () => {
  ...
  it('allows multiple score-mode entries per wallet and orders by score', () => {
    expect(
      simnet.callPublicFn(
        contract,
        'submit-score',
        [Cl.stringAscii('block_drop_multi'), Cl.uint(0), Cl.uint(5000), Cl.stringAscii('ACE')],
        wallet1
      ).result
    ).toBeOk(Cl.uint(1));
    ...
    expect(
      simnet.callPublicFn(
        contract,
        'submit-score',
        [Cl.stringAscii('block_drop_multi'), Cl.uint(0), Cl.uint(400), Cl.stringAscii('ACE')],
        wallet1
      ).result
    ).toBeOk(Cl.uint(2));
    ...
    expect(
      simnet.callPublicFn(
        contract,
        'submit-score',
        [Cl.stringAscii('block_drop_multi'), Cl.uint(0), Cl.uint(4500), Cl.stringAscii('ACE')],
        wallet1
      ).result
    ).toBeOk(Cl.uint(2));
    ...
    const board = simnet.callReadOnlyFn(
      contract,
      'get-top10',
      [Cl.stringAscii('block_drop_multi'), Cl.uint(0)],
      deployer
    ).result;
    const entries = unwrapTop10(board);
    expect(entries.length).toBe(3);
    expect(entries[0].player).toEqual(Cl.standardPrincipal(wallet1));
    expect(entries[0].score).toEqual(Cl.uint(5000));
    expect(entries[1].player).toEqual(Cl.standardPrincipal(wallet1));
    expect(entries[1].score).toEqual(Cl.uint(4500));
    expect(entries[2].player).toEqual(Cl.standardPrincipal(wallet1));
    expect(entries[2].score).toEqual(Cl.uint(400));
  });

  it('allows multiple time-mode entries and orders by lowest time', () => {
    expect(
      simnet.callPublicFn(
        contract,
        'submit-score',
        [Cl.stringAscii('maze_escape_multi'), Cl.uint(1), Cl.uint(5500), Cl.stringAscii('RUN')],
        wallet2
      ).result
    ).toBeOk(Cl.uint(1));
    ...
    expect(
      simnet.callPublicFn(
        contract,
        'submit-score',
        [Cl.stringAscii('maze_escape_multi'), Cl.uint(1), Cl.uint(5600), Cl.stringAscii('RUN')],
        wallet2
      ).result
    ).toBeOk(Cl.uint(2));
    ...
    expect(
      simnet.callPublicFn(
        contract,
        'submit-score',
        [Cl.stringAscii('maze_escape_multi'), Cl.uint(1), Cl.uint(5400), Cl.stringAscii('RUN')],
        wallet2
      ).result
    ).toBeOk(Cl.uint(1));
    ...
    const board = simnet.callReadOnlyFn(
      contract,
      'get-top10',
      [Cl.stringAscii('maze_escape_multi'), Cl.uint(1)],
      deployer
    ).result;
    const entries = unwrapTop10(board);
    expect(entries.length).toBe(3);
    expect(entries[0].score).toEqual(Cl.uint(5400));
    expect(entries[1].score).toEqual(Cl.uint(5500));
    expect(entries[2].score).toEqual(Cl.uint(5600));
  });
