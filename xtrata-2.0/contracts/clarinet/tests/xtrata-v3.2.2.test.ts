...
const contract = `${deployer}.xtrata-v3-2-2`;
const helperContract = `${deployer}.xtrata-small-mint-v1-1`;
const v2_1_0 = `${deployer}.xtrata-v2-1-0`;
const corePrincipal = Cl.contractPrincipal(deployer, 'xtrata-v3-2-2');
const mime = 'text/plain';
const contractSource = readFileSync(new URL('../contracts/xtrata-v3.2.2.clar', import.meta.url), 'utf8');
const liveContractSource = readFileSync(new URL('../../live/xtrata-v3.2.2.clar', import.meta.url), 'utf8');
...
describe('xtrata-v3.2.2 fixed 16 KiB core', () => {
  it('exposes fixed contract info and supports SIP-009 mint, transfer, URI, and enumeration', () => {
    ...
    expect(simnet.callReadOnlyFn(contract, 'get-contract-info', [], deployer).result).toBeOk(
      Cl.tuple({
        version: Cl.stringAscii('xtrata-v3.2.2'),
  ...
  it('migrates a legacy token even when its hash already has a first-seen token', () => {
    ...
    expect(simnet.callReadOnlyFn(v2_1_0, 'get-owner', [Cl.uint(legacyId)], deployer).result)
      .toBeOk(Cl.some(Cl.contractPrincipal(deployer, 'xtrata-v3-2-2')));
    expect(simnet.callReadOnlyFn(contract, 'get-id-by-hash', [Cl.bufferFromHex(hash)], deployer).result)
      .toBeSome(Cl.uint(10));
  });
  ...
  it('mints through the v3.2.2 small-mint helper with list 32 ABI and policy cap 30', () => {
    unwrapOk(setPaused(contract, false));
  ...
  it('migrates a v2.1.0 legacy token with same-id ownership, source lineage, and chunk fallback', () => {
    unwrapOk(setPaused(v2_1_0, false));
    unwrapOk(setPaused(contract, false));
    ...
    expect(simnet.callReadOnlyFn(contract, 'get-next-token-id', [], deployer).result)
      .toBeOk(Cl.uint(v210Id + 1n));
  });

  it('rejects re-migration of the same v2.1.0 token and preserves original ownership', () => {
    unwrapOk(setPaused(v2_1_0, false));
    unwrapOk(setPaused(contract, false));

    const v210Id = mintStaged(v2_1_0, wallet1, [byteHex(0xd0)], 'data:text/plain,v210-original');

    expect(simnet.callPublicFn(contract, 'migrate-from-v2-1-0', [Cl.uint(v210Id)], wallet1).result)
      .toBeOk(Cl.uint(v210Id));
    // Token is now minted in v3.2.2; second attempt fails with ERR-DUPLICATE before any transfer.
    expect(simnet.callPublicFn(contract, 'migrate-from-v2-1-0', [Cl.uint(v210Id)], wallet1).result)
      .toBeErr(Cl.uint(114));
