...
const marketName = "xtrata-market-sponsored-stx-v1-0";
...
describe("xtrata-market-sponsored-stx-v1.0", () => {
  ...
  it("buy transfers NFT and STX price, marks sold, keeps budget escrowed", () => {
    const tokenId = setup();
    ...
    const sellerStxBefore = stxBalance(seller);
    const ownerStxBefore = stxBalance(deployer);
    const buyerStxBefore = stxBalance(buyer);
    ...
    expect(stxBalance(seller)).toBe(sellerStxBefore + (price - fee));
    expect(stxBalance(deployer)).toBe(ownerStxBefore + fee);
    expect(stxBalance(buyer)).toBe(buyerStxBefore - price);
  ...
  it("conservation fuzz: random claim/refund orderings never mint or lose µSTX", () => {
    ...
    for (let round = 0; round < 5; round++) {
      ...
      const before =
        ...
        stxBalance(buyer) + stxBalance(deployer);
