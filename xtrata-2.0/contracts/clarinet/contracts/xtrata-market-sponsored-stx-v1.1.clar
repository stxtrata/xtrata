...
;;
;; STX escrow marketplace with seller-funded fee sponsorship.
;; Buyers pay the price in STX but need ZERO extra for fees; the seller
;; deposit covers the mining fee, so a buyer holding exactly the price can buy.
;; Extends xtrata-market-stx-v1.1:
...
;;
;; Clarinet/local lock targets: .xtrata-v2-1-0
...
;; Owner-managed allowlist of inscription cores this market accepts.
;; Seeded at deploy; extendable via set-nft-allowed when a new core ships,
;; so future cores never require a market redeploy. v3-only by policy:
;; v2 inscriptions must migrate to v3 before they can be listed.
(define-map AllowedNftContracts principal bool)
(map-set AllowedNftContracts .xtrata-v3-2-3 true)
(define-constant PRIMARY-NFT-CONTRACT .xtrata-v3-2-3)
...
(define-public (cancel (nft-contract <nft-trait>) (listing-id uint))
  (let ((listing (unwrap! (map-get? Listings listing-id) ERR-NOT-FOUND)))
    (begin
      (asserts! (nft-allowed (get nft-contract listing)) ERR-NOT-AUTHORIZED)
