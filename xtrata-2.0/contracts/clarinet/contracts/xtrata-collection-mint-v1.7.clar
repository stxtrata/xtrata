...
(define-constant ALLOWED-XTRATA-CONTRACT .xtrata-v3-2-3)
...
(define-private (assert-new-hash (hash (buff 32)))
  (begin
    (asserts! (is-none (contract-call? .xtrata-v3-2-3 get-id-by-hash hash)) ERR-DUPLICATE-HASH)
    (ok true)
  )
)
...
(define-private (core-begin-fee)
  (unwrap-panic (contract-call? .xtrata-v3-2-3 get-begin-fee-unit))
)
...
(define-private (core-seal-fee (chunk-count uint))
  (let (
    (chunk-unit (unwrap-panic (contract-call? .xtrata-v3-2-3 get-upload-chunk-fee-unit)))
    (batch-unit (unwrap-panic (contract-call? .xtrata-v3-2-3 get-upload-batch-fee-unit)))
    (seal-unit (unwrap-panic (contract-call? .xtrata-v3-2-3 get-seal-fee-unit)))
...
(define-private (core-upload-chunks (hash (buff 32)) (owner principal))
  (match (contract-call? .xtrata-v3-2-3 get-upload-state hash owner)
    ...
    (let (
      ...
      (begin
        ...
        (if new-session
          ...
          (if (is-none (contract-call? .xtrata-v3-2-3 get-upload-state expected-hash tx-sender))
  ...
  (let (
    ...
    (begin
      ...
      (let (
        ...
        (restart-fee (if (and resumed (is-none (contract-call? .xtrata-v3-2-3 get-upload-state expected-hash tx-sender)))
          (core-begin-fee) u0))
