...
(define-private (escrow-in (id uint) (player principal))
  (contract-call? .mock-xtrata-core transfer id player (as-contract tx-sender)) ;; @principal core
)

(define-private (payout (id uint) (winner principal))
  (as-contract (contract-call? .mock-xtrata-core transfer id tx-sender winner)) ;; @principal core
)
...
(define-private (inscription-hash-or-err (id uint))
  (ok (unwrap! (contract-call? .mock-xtrata-core get-inscription-hash id) ERR-HASH-MISSING)) ;; @principal core
)
