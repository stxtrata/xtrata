...
(define-private (deposit-one (id uint) (acc { role: uint, ok: bool }))
  (if (not (get ok acc))
    acc
    (let (
      (transferred (contract-call? 'SP3JNSEXAZP4BDSHV0DN3M8R3P0MY0EEBQQZX743X.xtrata-v3-2-3 transfer id tx-sender (as-contract tx-sender))) ;; @principal core
    )
      (match transferred
        success
          (begin
...
(define-private (payout (id uint) (recipient principal))
  (as-contract (contract-call? 'SP3JNSEXAZP4BDSHV0DN3M8R3P0MY0EEBQQZX743X.xtrata-v3-2-3 transfer id tx-sender recipient)) ;; @principal core
)
