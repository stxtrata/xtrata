;; xtrata-v3.2.4 (successor of v3.2.3: 128 MiB cap + migrate-from-v3-2-3; supersede via pause v3.2.3 + continuity set-next-id)
;;
;; Core posture (v3.2.3):
...
(define-constant MAX-TOTAL-CHUNKS u8192) ;; v3.2.4: 128 MiB (was u2048 in v3.2.3)
...
;; `recipient` is who ends up owning the inscription. It is NOT the payer.
;; maybe-pay always charges tx-sender, so the signer funds the mint and the
;; recipient receives it. Every pre-existing caller passes tx-sender here, which
;; reproduces v3.2.3 behaviour exactly. Only the new *-to entry points pass a
;; different principal. See docs: contracts/drafts/v3.2.4/MIGRATION-PLAN.md.
(define-private (commit-inscription
  ...
  (begin
    (try! (nft-mint? xtrata-inscription new-id recipient))
...
(define-public (migrate-from-v3-2-3 (token-id uint))
  (let (
    (meta (unwrap! (contract-call? .xtrata-v3-2-3 get-inscription-meta token-id) ERR-NOT-FOUND))
    (uri (contract-call? .xtrata-v3-2-3 get-token-uri-raw token-id))
    (deps (contract-call? .xtrata-v3-2-3 get-dependencies token-id))
    (parents (contract-call? .xtrata-v3-2-3 get-parents token-id))
    (hash (get final-hash meta))
  )
    (begin
      (try! (assert-inscription-allowed))
      (asserts! (is-none (nft-get-owner? xtrata-inscription token-id)) ERR-DUPLICATE)
      (try! (maybe-pay (var-get begin-fee-unit)))
      (try! (contract-call? .xtrata-v3-2-3 transfer token-id tx-sender (as-contract tx-sender)))
      (try! (nft-mint? xtrata-inscription token-id tx-sender))
      (map-insert InscriptionMeta token-id {
        owner: tx-sender,
        creator: (get creator meta),
      ...
      (record-hash-first-seen hash token-id)
      (match uri token-uri (map-set TokenURIs token-id token-uri) true)
      (if (> (len deps) u0) (map-set InscriptionDependencies token-id deps) true)
      (if (> (len parents) u0) (map-set InscriptionParents token-id parents) true)
      (record-migration-source token-id .xtrata-v3-2-3 token-id)
      (advance-next-id-if-needed token-id)
      (record-mint token-id)
      (print { event: "inscription-migrated", token-id: token-id, source-contract: .xtrata-v3-2-3, source-id: token-id, owner: tx-sender })
      (ok token-id)
    )
  )
)
...
(define-private (seal-internal
  ...
  (begin
    ...
    (let (
      ...
      ;; Staged path keeps v3.2.3 semantics: creator and owner are both the
      ;; signer. Sponsored staged seals are deliberately out of scope for 3.2.4.
      (commit-inscription
        expected-hash
        token-uri-string
        new-id
    ...
    (begin
      ...
      (let (
        ...
        (begin
          ...
          (try! (maybe-pay fee))
          ;; creator AND owner are the recipient. The payer is credited nowhere
          ;; on the token, only in the inscription-sealed event, which is what a
          ;; sponsored publishing flow wants: the author is the author.
...
(define-public (mint-single-tx
  ...
  (mint-single-tx-internal expected-hash mime total-size chunks token-uri-string (list) (list) tx-sender)
)
...
(define-public (mint-single-tx-recursive
  ...
  (mint-single-tx-internal expected-hash mime total-size chunks token-uri-string dependencies (list) tx-sender)
)
...
(define-public (mint-single-tx-with-relationships
  ...
  (mint-single-tx-internal expected-hash mime total-size chunks token-uri-string dependencies parents tx-sender)
)

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; --- SPONSORED MINTS (payer != owner) ---
;;
;; New in v3.2.4. The signer pays the protocol fee and the miner fee; `recipient`
;; receives the inscription and is recorded as its creator. This exists so a
;; publisher can fund an author's inscription in ONE transaction, without the
;; mint-then-transfer dance that leaves the funding wallet as first owner.
;;
;; Note for integrators: the STX post-condition still belongs to the signer, so
;; cap it with less-than-or-equal against quote-single-tx-fee as usual. The NFT
;; is minted directly to `recipient`, so a wallet that asserts NFT ownership
;; post-conditions must expect `recipient`, not the signer.
;;
;; `recipient` may be a standard or contract principal. There is deliberately no
;; consent check: minting an inscription TO someone is not a liability for them,
;; and requiring a signature from the recipient would defeat the entire purpose.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(define-public (mint-single-tx-to
  ...
  (mint-single-tx-internal expected-hash mime total-size chunks token-uri-string (list) (list) recipient)
)

(define-public (mint-single-tx-recursive-to
  (recipient principal)
  (expected-hash (buff 32))
  (mime (string-ascii 64))
  (total-size uint)
  ...
  (mint-single-tx-internal expected-hash mime total-size chunks token-uri-string dependencies (list) recipient)
)

(define-public (mint-single-tx-with-relationships-to
  (recipient principal)
  (expected-hash (buff 32))
  (mime (string-ascii 64))
  (total-size uint)
  ...
  (mint-single-tx-internal expected-hash mime total-size chunks token-uri-string dependencies parents recipient)
)
