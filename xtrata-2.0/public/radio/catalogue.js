...
const columns=[['id','ID'],['title','Song'],['album','Album'],['artist','Artist'],['plays','Plays'],['onchain_likes','♥ On-chain likes'],['current_likes','Saved favourites'],['completions','Completed'],['duration','Duration (seconds)'],['starts','Starts'],['partials','Partial'],['in_progress','In progress'],['repeats','Repeats'],['unique_browsers','Unique browsers'],['seconds','Listening minutes'],['completion_rate','Completion %'],['last_play','Last play'],['creator','Creator'],['status','Catalogue status']];
let rows=[],sort='id',direction=1,requestId=0;
let selected=new URL(location.href).searchParams.get('id');
const display=(row,key)=>{const v=row[key];if(v===null||v===undefined||v==='')return '—';if(key==='onchain_likes')return '♥ '+String(v);if(key==='last_play')return new Date(v).toLocaleString();if(key==='seconds')return (v/60).toFixed(1);if(key==='completion_rate'||key==='duration')return Number(v).toFixed(1);return String(v);};
...
function detail(row){
 ...
 const a=document.createElement('a');a.href='/radio/embed?tokenId='+row.id;a.textContent='Listen to this song';el('detail').append(a);const like=document.createElement('a');like.href='/radio/endorse?id='+row.id;like.textContent=' · Like or unlike on-chain';el('detail').append(like);
 history.replaceState(null,'','?id='+row.id);
}
function render(){
 const visible=el('more').checked?columns:columns.slice(0,8);el('columns').replaceChildren();el('songs').replaceChildren();
 ...
 const filtered=rows.filter(r=>[r.id,r.title,r.album,r.artist,r.creator].some(v=>String(v||'').toLowerCase().includes(query)));
...
async function load(){const id=++requestId;el('status').textContent='Loading…';try{const response=await fetch('/radio/counts?range='+el('period').value,{cache:'no-store'});const data=await response.json();if(!response.ok)throw Error(data.error||'Statistics unavailable');if(id!==requestId)return;rows=data.tracks;el('status').textContent=`${rows.length} catalogue entries · Measured since ${new Date(data.measured_since).toLocaleDateString()} · Updated ${new Date(data.until).toLocaleTimeString()}`;el('notice').textContent=(data.onchain_likes_status==='partial'?'Some on-chain totals could not be loaded. Available totals are shown; use Refresh to retry the missing ones. ':data.onchain_likes_status==='unavailable'?'On-chain totals are temporarily unavailable. Your recorded likes are unaffected; use Refresh to retry. ':data.onchain_likes_status==='disabled'?'On-chain likes are not enabled in this environment. ':'')+data.notice;render();const row=rows.find(r=>String(r.id)===selected);if(row)detail(row);}catch(e){if(id!==requestId)return;rows=[];render();el('detail').hidden=true;el('status').textContent=e.message;}}
