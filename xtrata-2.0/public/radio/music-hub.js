...
fetch('/radio/music-releases.json',{cache:'no-cache'}).then(r=>{if(!r.ok)throw Error();return r.json();}).then(release=>{
 ...
 const host=document.getElementById('downloads-list');
 const betaHost=document.getElementById('beta-downloads-list');
 if(verified.some(d=>!d.beta))host.replaceChildren();
 if(betaHost&&verified.some(d=>d.beta))betaHost.replaceChildren();
 for(const d of verified){const card=document.createElement('article'),title=document.createElement('h3'),p=document.createElement('p'),link=document.createElement('a'),details=document.createElement('details'),summary=document.createElement('summary'),hash=document.createElement('code');title.textContent=platforms[d.platform];p.textContent=d.requirements||'See release notes for compatibility.';link.className='button';link.href=d.url;link.textContent=(d.beta?'Download beta ':d.preview?'Download test preview ':'Download ')+versionOf(d)+(d.beta&&Number.isSafeInteger(d.betaBuild)&&d.betaBuild>0?' · build '+d.betaBuild:'');summary.textContent='Verify download (SHA-256)';hash.textContent=d.sha256;details.append(summary,hash);card.append(title,p,link,details);if(d.preview){const note=document.createElement('p');note.textContent=previewNote(d.platform);card.append(note);}if(d.beta){const row=document.createElement('details'),heading=document.createElement('summary');row.className='beta-download-row';link.className='beta-download-link';link.textContent=platforms[d.platform]+' · '+versionOf(d)+(d.betaBuild?' · build '+d.betaBuild:'');heading.append(link);row.append(heading,p,details);for(const note of Array.from(card.querySelectorAll(':scope > p')))row.append(note);betaHost?.append(row);}else host?.append(card);}
}).catch(()=>{});
