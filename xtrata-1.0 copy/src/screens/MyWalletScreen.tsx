...
export default function MyWalletScreen(props: MyWalletScreenProps) {
  ...
  const manualEntered = trimmedManual.length > 0;
  const manualValid =
    trimmedManual.length === 0 || validateStacksAddress(trimmedManual);
  const targetAddress =
    manualEntered && manualValid
      ? trimmedManual
      : walletAddress ?? '';
  const showEmptySearch = searchTouched && !manualEntered && !walletAddress;
  const showInvalidSearch = manualEntered && !manualValid;
    ...
    >
      <div className="panel">
        <div className="panel__header">
          <div>
            <h2>My wallet</h2>
            <p>Search by address or use the connected wallet to view holdings.</p>
          </div>
          ...
          <div className="grid-panel">
            <div className="grid-panel__meta">
              <div className="wallet-lookup">
                <div className="wallet-lookup__title">
                  <h3>Wallet lookup</h3>
                  <p>Search by Stacks wallet address to view owned inscriptions.</p>
                </div>
                <form
                ...
                <div className="wallet-lookup__status">
                  {walletAddress ? (
                    ...
                    <>
                      <span className="meta-label">Connected wallet</span>
                      <span className="meta-value">Not connected</span>
                    </>
                  )}
                </div>
                {targetAddress && (
                  <div className="wallet-lookup__status">
                    ...
                    <span className="meta-value">{targetAddress}</span>
                  </div>
                )}
              </div>
