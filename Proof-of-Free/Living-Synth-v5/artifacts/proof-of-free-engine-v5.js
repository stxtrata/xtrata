  ...
  function createMosaic(canvas, opts = {}) {
    ...
    function frame(now) {
      ...
      for (const g of genomes) {
        ...
        const amp = m.fadeAmp * (isLive || isDraining ? 1 : 0.68);
        let light = m.fade === "white" ? baseL + (96 - baseL) * tt * amp
                                       : baseL - (baseL - floorL) * tt * amp;
        light = Math.max(2, Math.min(97, light + (isLive || isDraining ? 8 : 0) + pulse * 3 * dr.vibrancy));
        const sat = Math.max(0, m.sat * (1 - 0.32 * tt * amp));
        ...
        if (isDrumTile) drawDrumGlyph(c, g, px, py, s, time, pulse, isLive || isDraining, inst);
        else if (!(m.family === "black" && m.brightness < 0.09 && !isLive && !isDraining && dr.vibrancy < 0.32)) {
          drawTileMotif(c, an, m, px, py, s, time, isLive || isDraining ? 1 : 0.44 + dr.vibrancy * 0.2, inst);
        }
    ...
    function drawDrumGlyph(c, g, px, py, s, time, pulse, isActive, inst) {
      ...
      c.globalAlpha = (isActive ? 0.55 : 0.13) * (1 - kick * 0.7);
      ...
      c.scale(sc, sc);
      c.globalAlpha = isActive ? 0.85 + 0.15 * lvl : 0.28 + 0.15 * pulse;
