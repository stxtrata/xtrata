...
function mint(id: number, owner: string, mime: string, parents: number[] = []) {
...
describe("living synth child registry", () => {
  ...
  it("registers incremental edition batches atomically and rejects collisions", () => {
    ...
    const duplicate = Cl.list([
      Cl.tuple({ edition: Cl.uint(1), "nft-id": Cl.uint(100) }),
      Cl.tuple({ edition: Cl.uint(1), "nft-id": Cl.uint(101) })
    ]);
    ...
    const valid = Cl.list([
      Cl.tuple({ edition: Cl.uint(1), "nft-id": Cl.uint(100) }),
      Cl.tuple({ edition: Cl.uint(2), "nft-id": Cl.uint(101) })
    ]);
