;; Proof of Free - Living Synth registry v2
;;
...
(define-constant ERR-ENGINE-LOCKED (err u120))
(define-constant ERR-INVALID-EDITION-CONTENT (err u121))
(define-constant ERR-ENGINE-DEPENDENCY-MISSING (err u122))
(define-constant ERR-REGISTRATION-NOT-PAUSED (err u123))

(define-trait xtrata-core-trait
  (
    (get-owner (uint) (response (optional principal) uint))
    (get-parents (uint) (response (list 50 uint) uint))
    (get-dependencies (uint) (response (list 50 uint) uint))
...
(define-map EditionContentHashes uint (buff 32))
...
(define-private (write-edition (entry { edition: uint, nft-id: uint, content-hash: (buff 32) }))
  (let (
    (edition (get edition entry))
    (nft-id (get nft-id entry))
    (content-hash (get content-hash entry))
  )
    ...
    (map-set EditionContentHashes edition content-hash)
...
(define-private (write-edition-step
  (entry { edition: uint, nft-id: uint, content-hash: (buff 32) })
...
(define-public (set-engine (core <xtrata-core-trait>) (inscription-id uint))
  (begin
    ...
    (asserts!
      (and
        (is-none (var-get engine-id))
        (is-eq (var-get registered-editions) u0)
        (var-get paused)
      )
      ERR-ENGINE-LOCKED
    )
...
(define-public (register-edition
  ...
  (begin
    (try! (assert-owner))
    (try! (assert-core core))
    (asserts! (var-get paused) ERR-REGISTRATION-NOT-PAUSED)
    (let (
      (locked-engine-id (unwrap! (var-get engine-id) ERR-INVALID-ENGINE))
      (meta (unwrap! (try! (contract-call? core get-inscription-meta nft-id)) ERR-NOT-FOUND))
      (dependencies (try! (contract-call? core get-dependencies nft-id)))
    )
      ...
      (asserts!
        (and
          (get sealed meta)
          (> (get total-size meta) u0)
          (<= (get total-size meta) MAX-EDITION-BYTES)
          (is-eq (get mime-type meta) EDITION-MIME)
        )
        ERR-INVALID-EDITION-CONTENT
      )
      (asserts! (is-some (index-of dependencies locked-engine-id)) ERR-ENGINE-DEPENDENCY-MISSING)
      (write-edition {
        edition: edition,
        nft-id: nft-id,
        content-hash: (get final-hash meta)
...
(define-public (register-edition-batch
  (entries (list 25 { edition: uint, nft-id: uint, content-hash: (buff 32) }))
)
  (begin
    (try! (assert-owner))
    (asserts! (var-get paused) ERR-REGISTRATION-NOT-PAUSED)
    (asserts! (is-some (var-get engine-id)) ERR-INVALID-ENGINE)
...
(define-read-only (get-contract-version)
  "proof-of-free-living-synth/2.0.0"
)

(define-read-only (get-edition-content-hash (edition uint))
  (map-get? EditionContentHashes edition)
)
