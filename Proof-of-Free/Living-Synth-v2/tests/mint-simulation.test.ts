...
describe("full collection mint simulation", () => {
  it("mints, registers, records, transfers, and audits all 1,024 editions", { timeout: 120_000 }, async () => {
    ...
    expect(mint(
      ...
      [],
      [],
      Cl.bufferFromHex(release.manifest.engine.sha256)
    ).result).toBeOk(Cl.uint(ENGINE_ID));
    ...
    for (let edition = 1; edition <= COLLECTION_SIZE; edition += 1) {
      ...
      const child = release.children[edition - 1];
      expect(mint(
        nftId,
        owner,
        "text/html",
        [],
        [ENGINE_ID],
        Cl.bufferFromHex(child.sha256)
      ).result).toBeOk(Cl.uint(nftId));
      expect(read(GATEWAY, "get-owner", [Cl.uint(nftId)], owner))
        .toBeOk(Cl.some(Cl.standardPrincipal(owner)));
      expect(read(GATEWAY, "get-dependencies", [Cl.uint(nftId)], owner))
        .toBeOk(Cl.list([Cl.uint(ENGINE_ID)]));
    }
    ...
    for (let start = 1; start <= COLLECTION_SIZE; start += BATCH_SIZE) {
      const entries = Array.from(
        ...
        (_, offset) => {
          ...
          return Cl.tuple({
            edition: Cl.uint(edition),
            "nft-id": Cl.uint(nftForEdition(edition)),
            "content-hash": Cl.bufferFromHex(release.children[edition - 1].sha256)
          });
        }
      );
    ...
    for (let page = 0; page < COLLECTION_SIZE / PAGE_SIZE; page += 1) {
      ...
      cells.forEach((cell, offset) => {
        ...
        expect(read(REGISTRY, "get-edition-content-hash", [Cl.uint(edition)]))
          .toBeSome(Cl.bufferFromHex(release.children[edition - 1].sha256));
      });
