...
;; 6) Upload sessions expire after inactivity (block-height based) and are purgeable:
...
;;
;; v10.0.2 upgrades (boring infra dedupe):
;; - Canonical-only content: a given final-hash can be sealed at most once
;; - HashToId: on-chain lookup (hash -> token-id)
;; - Early duplicate detection at begin-inscription (prevents wasted chunk uploads)
;; - Race-safety at seal (reject if hash already sealed)
;; - begin-or-get helper: one-call UX for "start upload OR return canonical token"
...
(define-constant ERR-DUPLICATE          (err u114)) ;; NEW
...
;; Upload expiry (~1 year at 10-min block cadence)
(define-constant UPLOAD-EXPIRY-BLOCKS u52560)
...
(define-private (upload-expired?
  ...
  (>= block-height (+ (get last-touched state) UPLOAD-EXPIRY-BLOCKS))
)
...
;;
;; v10.0.2: hard dedupe early detection:
      ...
      state
        (begin
          ...
          (map-set UploadState
            ...
            (merge state { last-touched: block-height })
          )
          (ok true)
        )
      (begin
...
(define-private (seal-internal (expected-hash (buff 32)) (token-uri-string (string-ascii 256)) (new-id uint))
  (let (
    ...
    (begin
      ...
      ;; NEW: race-safe canonical-only seal
      (asserts! (is-none (map-get? HashToId expected-hash)) ERR-DUPLICATE)
      ...
      ;; NEW: record canonical mapping (hash -> id)
