...
const contract = `${deployer}.xstrata-v1-0-5`;
...
describe("xstrata-v1.0.5 contract", () => {
  it("starts paused and only owner can inscribe", () => {
    ...
    const ownerSeal = sealInscription(deployer, expectedHash, "ipfs://xstrata/owner-paused");
  ...
  it("supports resume and exposes upload state", () => {
    ...
    const unauthorizedSeal = sealInscription(wallet2, expectedHash, "ipfs://xstrata/unauth");
  ...
  it("requires full upload and non-empty uri to seal", () => {
    ...
    const sealIncomplete = sealInscription(wallet1, expectedHash, "ipfs://xstrata/incomplete");
    ...
    const sealOk = sealInscription(wallet1, expectedHash, "ipfs://xstrata/complete");
    expect(sealOk.result).toBeOk(Cl.uint(0));

    const uri = simnet.callReadOnlyFn(contract, "get-token-uri", [Cl.uint(0)], deployer);
    expect(uri.result).toBeOk(Cl.some(Cl.stringAscii("ipfs://xstrata/complete")));
  });
  ...
  it("transfers fees to the royalty recipient across batches", () => {
    ...
    const seal = sealInscription(wallet1, expectedHash, "ipfs://xstrata/fees");
    expect(seal.result).toBeOk(Cl.uint(0));
  ...
  it("rejects seal when the hash mismatches", () => {
    ...
    const seal = sealInscription(wallet1, expectedHash, "ipfs://xstrata/bad");
  ...
  it("blocks inscription writes when paused but allows transfers", () => {
    ...
    beginInscription(wallet1, expectedHash, 1, 1);
    addChunkBatch(wallet1, expectedHash, [chunkHex]);
    sealInscription(wallet1, expectedHash, "ipfs://xstrata/pause-0");
    ...
    const blockedSeal = sealInscription(wallet1, blockedHash, "ipfs://xstrata/pause-blocked");
  ...
  it("rejects unauthorized transfers", () => {
    ...
    beginInscription(wallet1, expectedHash, 1, 1);
    addChunkBatch(wallet1, expectedHash, [chunkHex]);
    sealInscription(wallet1, expectedHash, "ipfs://xstrata/transfer");
