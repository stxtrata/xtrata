...
type RecursiveRequestMessage = {
  type: 'xstrata:recursive:request';
  bridgeId: string;
  requestId: string;
  functionName: string;
...
type RecursiveResponseMessage = {
  type: 'xstrata:recursive:response';
  bridgeId: string;
  requestId: string;
  ok: boolean;
  result?: string;
...
const handleMessage = async (event: MessageEvent<RecursiveRequestMessage>) => {
  ...
  if (!data || data.type !== 'xstrata:recursive:request') {
    return;
  }
...
const buildBridgeScript = (bridgeId: string) => {
  ...
  return `<script data-xstrata-bridge="true">
(function(){
  if (window.__xstrataRecursiveBridge) { return; }
  ...
  window.addEventListener('message', (event) => {
    const data = event.data || {};
    if (data.type !== 'xstrata:recursive:response' || data.bridgeId !== bridgeId) {
      return;
    }
  ...
  window.__xstrataRecursiveBridge = { bridgeId };
})();
</script>`;
};
...
export const injectRecursiveBridgeHtml = (html: string, bridgeId: string) => {
  ...
  if (html.includes('data-xstrata-bridge')) {
    return html;
  }
