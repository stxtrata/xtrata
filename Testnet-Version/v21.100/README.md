...
- Enable in the browser console:
  - `localStorage.setItem('xstrata.log.level', 'debug')`
  - `localStorage.setItem('xstrata.log.tags', 'chunk,preview,token-uri,stream,cache,tab,mint')`
  - `localStorage.setItem('xstrata.log.enabled', 'true')`
- Or via env vars:
